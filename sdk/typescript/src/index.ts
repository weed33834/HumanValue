/**
 * HumanValue 开放 API 的 TypeScript SDK（WS-3）
 *
...
export interface VerifyOptions {
  ...
  body: string;
  /** `X-HumanValue-Signature` 头原值（`t=...,v1=...`），或仅 v1 hex */
  signature: string;
  /** 容忍窗口（s），<=0 表示不校验时间 */
...
export class HumanValueError extends Error {}

/** 平台返回 4xx（不重试） */
export class ApiError extends HumanValueError {
  constructor(
    public readonly statusCode: number,
    public readonly detail: unknown,
  ) {
    super(`HumanValue API 错误: HTTP ${statusCode} ${JSON.stringify(detail)}`);
    this.name = "ApiError";
  }
}

/** 429 / 5xx 重试耗尽 */
export class RetryableError extends HumanValueError {
  ...
  private async request<T>(
    ...
    for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
      ...
      try {
        ...
        throw new HumanValueError(`网络请求失败: ${String(err)}`);
      }
      if (response.ok) {
...
export default HumanValueClient;
