"""HumanValue 开放 API 的 Python SDK（WS-3）
...
.. code-block:: python
    ...
    valid = verify_webhook_signature(
        ...
        signature="t=1754630400,v1=9f86d0...",   # X-HumanValue-Signature 原值
    )
"""
...
class HumanValueError(Exception):
    """SDK 基础异常。"""


class ApiError(HumanValueError):
    """平台返回非 2xx（不重试）。携带状态码与响应体。"""

    def __init__(self, status_code: int, detail: Any):
        super().__init__(f"HumanValue API 错误: HTTP {status_code} {detail!r}")
        self.status_code = status_code
        self.detail = detail


class RetryableError(HumanValueError):
    """重试耗尽仍失败（429 / 5xx）。"""
...
def verify_webhook_signature(
    ...
    *,
    ...
    Args:
        ...
        signature: ``X-HumanValue-Signature`` 头原值（``t=...,v1=...``），
            也可只传 ``v1`` 的 hex。
