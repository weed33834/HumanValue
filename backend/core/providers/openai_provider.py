    ...
    async def stream_chat_completion(
        ...
        for attempt in range(MAX_RETRIES + 1):
            try:
                ...
                async for chunk in _anext_with_idle_timeout(stream, idle_timeout):
                ...
                # 流式空闲超时: 终止, 由调用方降级/报错, 绝不无限阻塞
                logger.warning(
                    "stream_chat_completion 空闲超时 (%ss) model=%s, 终止流式",
                    _stream_idle_timeout(),
                    self.config.model_name,
                )
                self._safe_metric(record_llm_request, self._tier, "timeout")
                raise
            except Exception as e:
                err_str = str(e)
                is_rate_limit = (
...
async def _anext_with_idle_timeout(stream, idle_timeout: float):
    """对流式迭代器做空闲超时: 每个 chunk 之间若超过 idle_timeout 秒无数据, 抛 TimeoutError。

    防止上游(聚合通道/慢模型)挂起导致聊天/评估无限阻塞。
    """
    while True:
        try:
            ...
            raise asyncio.TimeoutError(f"流式空闲超时 (>{idle_timeout}s 无数据)")
        yield chunk
...
def _stream_idle_timeout() -> float:
    ...
    try:
        ...
        return float(getattr(get_settings(), "llm_stream_idle_timeout", 90.0))
    except Exception:
        return 90.0
