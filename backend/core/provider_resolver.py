...
async def get_provider_for_model(model_name: str) -> Optional[BaseProvider]:
    ...
    优先级：
    0. Mock 模式：若 LLM_MOCK_MODE=true，直接返回 MockProvider（无需任何模型凭证）
    ...
    # 0. Mock 模式：直接返回 MockProvider，不调用任何真实 API
    if getattr(settings, "llm_mock_mode", False):
        logger.info("LLM_MOCK_MODE 已开启，使用 MockProvider 替代 %s", model_name)
...
async def _lookup_tenant_credential_for_model(
    ...
    async with get_db_session() as sess:
        ...
        if tm:
            ...
            # 查找该租户启用的首个 provider 及其活跃凭证 (显式关联, 避免笛卡尔积)
            cstmt = (
            ...
            for tp in cresult.scalars().all():
                if tp.active_credential_id:
                    provider_name = tp.provider
                    active_cred_id = tp.active_credential_id
                    break
            else:
                return None, None
