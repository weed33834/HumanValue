...
async def _on_gitlab_issue(payload: Dict[str, Any]) -> None:
    ...
    logger.info(
        "事件订阅: GitLab Issue repo=%s title=%s action=%s", repo, title, action
    )
...
def register_event_subscribers() -> None:
    ...
    _unsubscribers.append(bus.subscribe("webhook:feishu:message", _on_feishu_message))
    _unsubscribers.append(bus.subscribe("webhook:feishu:card", _on_feishu_card))
    _unsubscribers.append(bus.subscribe("webhook:gitlab:push", _on_gitlab_push))
    _unsubscribers.append(bus.subscribe("webhook:gitlab:mr", _on_gitlab_mr))
    _unsubscribers.append(bus.subscribe("webhook:gitlab:issue", _on_gitlab_issue))
    # 自定义 webhook 通用订阅者
    _unsubscribers.append(bus.subscribe("webhook:custom", _on_custom_webhook))
