...
async def _invoke_provider(
    ...
    *,
    ...
    # M23 缓存: 命中直接返回 (不重复调用/计费)
    cache_hit = await _try_cache_get(messages, model_name, response_format)
    ...
    async with AsyncExitStack() as stack:
        ...
        await _try_cache_set(messages, model_name, completion.content, response_format)
        return completion
...
async def _cache_enabled() -> bool:
    try:
        ...
        return bool(getattr(get_settings(), "llm_cache_enabled", False))
    except Exception:
        return False


async def _try_cache_get(messages, model, response_format):
    if not await _cache_enabled():
        return None
    try:
        from core.llm_cache import get_global_llm_cache

        return await get_global_llm_cache().get(messages, model, response_format)
    except Exception as e:
        logger.debug("LLM 缓存读取失败: %s", e)
        return None


async def _try_cache_set(messages, model, content, response_format):
    if not await _cache_enabled() or not content:
        return
    try:
        from core.llm_cache import get_global_llm_cache

        await get_global_llm_cache().set(messages, model, content, response_format)
    except Exception as e:
