...
class TaskScheduler:
    ...
    async def _execute_task(
        ...
        try:
            ...
            # SQLite 并发写锁（database is locked）属可重试的临时性冲突，
            # 下一个调度周期会自然重试，不打印堆栈刷屏，也不记失败。
            if "database is locked" in str(e) or "database is locked" in str(
                e.__cause__ or ""
            ):
                status_str = "retry_later"
                error_msg = "SQLite 并发写锁冲突，下个周期重试"
                logger.warning("任务 %s 遇 SQLite 写锁冲突，下个周期重试", task_id)
            else:
                status_str = "failed"
                error_msg = str(e)
