...
# 低分阈值：低于或等于该分数视为高风险，需路由 HR 审核
RISK_SCORE_THRESHOLD = 60.0
# 需要路由 HR 审核的风险等级
HR_AUDIT_RISK_LEVELS = frozenset({"high", "critical"})


def _route_by_risk(evaluation) -> str:
    """根据评估结果进行风险分级路由，决定审批流向。

    规则：
    - overall_score <= RISK_SCORE_THRESHOLD(60) 或存在 high/critical 风险标记 → HR_AUDIT
    - 其余（正常评估）→ MANAGER_REVIEW
    返回值为评估图的最终状态（路由信号），由 API 层据此驱动审批状态机。
    """
    score = getattr(evaluation, "overall_score", None)
    risk_flags = []
    mgr_view = getattr(evaluation, "manager_view", None)
    if mgr_view is not None:
        risk_flags = list(getattr(mgr_view, "risk_flags", None) or [])

    high_risk_flag = any(
        getattr(flag, "level", None) in HR_AUDIT_RISK_LEVELS for flag in risk_flags
    )
    if (score is not None and score <= RISK_SCORE_THRESHOLD) or high_risk_flag:
        return EvaluationStatus.HR_AUDIT
...
def _make_call_llm(
    model_router: ModelRouter, prompt_loader: PromptLoader, prompt_name: str
):
    ...
    async def parse_output(state: EvaluationState) -> EvaluationState:
        ...
        with _node_trace("parse_output", state):
            ...
            try:
                ...
                # 风险分级路由：决定审批流向（API 层据此驱动状态机）
                # - 低分(<=60)或存在 high/critical 风险标记 → 高风险，路由 HR 审核
                # - 其余 → 常规，路由主管审批
                routing = _route_by_risk(evaluation)
                return {
                    **state,
...
def _make_review_gate():
    """构建审批闸门节点（interrupt 版专用）。

    在 parse_output 之后暂停，等待人工审批（manager 或 HR）。
    - 首次执行调用 interrupt({"node": node_name, "action": "awaiting_review"}) 暂停
    - resume 时收到 {"action": "approve"|"reject"|"request_hr_review", "comment": "...", "actor_id": "..."}
    - 根据 action 设置评估最终状态并返回
    """
    from langgraph.types import interrupt

    async def review_gate(state: EvaluationState) -> EvaluationState:
        if state.get("error"):
            return state

        parsed = state.get("parsed_evaluation")
        if not parsed:
            return {**state, "error": "缺少解析结果，无法进入审批", "status": "error"}
        ...
        action = (
            (approval or {}).get("action")
            if isinstance(approval, dict)
            else (approval or "approve")
        )
        comment = approval.get("comment", "") if isinstance(approval, dict) else ""
        actor_id = approval.get("actor_id", "") if isinstance(approval, dict) else ""

        # 更新评估状态
        new_parsed = dict(parsed)
        if action == "approve":
            new_parsed["status"] = EvaluationStatus.APPROVED
            if actor_id:
                new_parsed["approver_id"] = actor_id
        elif action == "reject":
            new_parsed["status"] = EvaluationStatus.REJECTED
        elif action == "request_hr_review":
            new_parsed["status"] = EvaluationStatus.HR_AUDIT
        else:
            new_parsed["status"] = EvaluationStatus.APPROVED

        # 记录审批信息到 audit
        audit = dict(new_parsed.get("audit") or {})
        audit["approval"] = {
            "action": action,
            "comment": comment,
            "actor_id": actor_id,
        ...
        new_parsed["audit"] = audit

        return {
            **state,
            "parsed_evaluation": new_parsed,
...
def create_evaluation_graph(
    ...
    builder.add_node(
        "data_cleaning", _make_data_cleaning(multimodal_cleaner, input_guard)
    )
    ...
    builder.add_node(
        "call_llm", _make_call_llm(model_router, prompt_loader, prompt_name)
    )
    ...
    builder.add_edge("parse_output", "finalize")
...
def create_evaluation_graph_with_interrupt(
    ...
    builder.add_node("parse_output", _make_parse_output(output_guard))
    builder.add_node("review_gate", _make_review_gate())
    builder.add_node("finalize", _finalize)
    ...
    builder.add_edge("build_prompt", "call_llm")
    builder.add_edge("call_llm", "parse_output")
    builder.add_edge("parse_output", "review_gate")
