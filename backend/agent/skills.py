...
class SkillExecutor:
    ...
    async def _execute_with_react(
        ...
        try:
            ...
            tool_map: Dict[str, Any] = {getattr(t, "name", ""): t for t in all_tools}
    ...
    async def generate_skill(self, description: str, category: str = "general") -> dict:
        ...
        try:
            ...
            if skill_def is None:
                logger.warning(
                    "generate_skill LLM 输出无法解析为 JSON: %s", output_text[:200]
                )
                return {
    ...
    async def import_skill(self, skill_data: dict, overwrite: bool = False) -> dict:
        """从 JSON 数据导入 Skill
