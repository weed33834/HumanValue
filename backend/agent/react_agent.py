...
async def build_all_tools(
    ...
    # 5) Browser 浏览器自动化工具 (M4.8, Playwright 可选)
    browser_tools_count = await _load_browser_tools(tools, enabled_csv)
    if browser_tools_count:
        logger.info("加载 %d 个 Browser 工具", browser_tools_count)

    # 6) 控制工具 (让对话可调用业务服务: 人才分析/公告/工单/管道/容灾/安全)
    control_tools_count = await _load_control_tools(tools, enabled_csv)
    if control_tools_count:
        logger.info("加载 %d 个控制工具", control_tools_count)

    return tools


async def _load_control_tools(tools: List[Any], enabled_csv: Optional[str]) -> int:
    """加载控制工具 (对话执行业务服务), 由 enabled_tools 中的 control_* 或 control_tools_enabled 控制。"""
    try:
        from agent.control_tools import _load_control_tools as _build
    except ImportError:
        return 0
    try:
        control_tools = _build()
    except Exception as e:
        logger.warning("构建控制工具失败: %s", e)
        return 0
    count = 0
    for t in control_tools:
        ...
        if not name:
            continue
        # 允许 enabled_tools 精确关闭 (如 control_tools=false)
        if enabled_csv and f"{name}=false" in enabled_csv:
            continue
        tools.append(t)
        count += 1
    return count
...
async def _load_browser_tools(tools: List[Any], enabled_csv: Optional[str]) -> int:
    """加载 Playwright 浏览器自动化工具 (browser_navigate / browser_click 等)。"""
    try:
        from agent.browser_tool import build_browser_tools
    except ImportError:
        return 0
    try:
        browser_tools = build_browser_tools()
    except Exception as e:
        logger.warning("构建 Browser 工具失败: %s", e)
        return 0
    ...
    for t in browser_tools:
        name = getattr(t, "name", "")
        if not name or not name.startswith("browser_"):
            continue
        if enabled_csv and name not in {
            x.strip() for x in enabled_csv.split(",") if x.strip()
        }:
            continue
        tools.append(t)
        count += 1
...
def _build_skill_tool(skill: Any) -> Optional[Any]:
    ...
    args_schema = _json_schema_to_pydantic(skill.input_schema, f"{skill_name}_input")
    ...
    def _run(**kwargs) -> str:
        ...
        try:
            ...
            return asyncio.run(_invoke_skill_tool(skill_id, skill_name, dict(kwargs)))
...
def create_evaluation_react_agent(
    ...
    async def _build():
        ...
        if system_prompt is None:
            default_sp = (
                "你是 HumanValue 评估助手，服务于管理者（老板）视角。你可以调用工具获取员工历史、"
                "查询公司知识库,进行综合分析后给出VGR（价值-成长-风险）评估建议。"
                "回答时引用具体数据与证据,避免臆测。"
            )
        else:
