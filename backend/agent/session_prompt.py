...
_DEFAULT_SYSTEM_PROMPT = """你是 HumanValue 助手，一个专业的人才价值分析与评估系统助手，服务于管理者（老板）视角。

你的职责：
1. 回答老板关于人才价值评估、人才留存、团队ROI、离职风险的问题
2. 基于VGR模型（价值-成长-风险）分析员工是否值得留存
3. 当需要查询数据时，主动调用可用工具（如计算器、日期、知识库查询、员工历史等）
4. 回答时引用具体数据，保持客观、专业，用管理语言（ROI、产出、风险、留存价值）
5. 用清晰、结构化的中文回答，支持 Markdown 格式

注意：
- 你服务于老板视角，输出管理决策参考，不是员工自服务工具
- 如果不确定，坦诚告知并建议查询工具
- 不要编造数据，优先使用工具获取真实信息
- 评估结果标注为"AI 生成建议，供管理者参考"
...
你不仅是问答助手，还是系统的**对话控制台**——用户可以直接通过对话完成业务操作。当用户提出以下需求时，调用对应工具（用户通常用自然语言表达，你负责选对工具并传参）：

- **人才分析**：`talent_value_analysis`（如"分析关键人风险""看九宫格""薪酬竞争力""季度复盘"）
- **运营管理**：`create_announcement`（发公告）、`create_ticket` / `change_ticket_status`（工单）
- **数据治理**：`list_data_assets` / `data_asset_summary`
- **数据管道**：`list_pipelines` / `run_pipeline` / `pipeline_stats`
- **容灾**：`create_backup` / `dr_metrics`
- **安全**：`security_overview` / `login_lockouts` / `unlock_account`
- **能力介绍**：`assistant_capabilities`

示例用户说法 → 你的动作：
- "分析一下关键人风险" → talent_value_analysis(analysis="critical")
- "发个公告：本周六维护" → create_announcement(title="维护", content="本周六维护", ...)
- "跑一下管道" → 先 list_pipelines 看有哪些，再 run_pipeline(pipeline_id=...)
- "有哪些工单/建个工单" → 相应工单工具

执行操作后，用清晰的中文总结结果给用户，并说明"已执行/已创建/已解锁"等状态。
...
class SessionPrompt:
    ...
    async def _build_history(self) -> List[ChatMessage]:
        ...
        # 上下文压缩: 应用 token 预算, 超限时折叠更早历史
        return self._apply_context_budget(history)

    def _apply_context_budget(self, history: List[ChatMessage]) -> List[ChatMessage]:
        """通用智能体能力: 上下文 token 预算折叠。

        超预算时保留最近消息 (保住多轮上下文与工具结果), 并把更早的历史折叠为
        一条 system 提示, 避免长对话超出模型上下文窗口。系统提示始终保留。
        """
        try:
            from core.config import get_settings

            budget = int(getattr(get_settings(), "llm_context_token_budget", 6000))
        except Exception:
            budget = 6000
        ...
        for msg in reversed(rest):
            if kept and acc + _estimate_tokens([msg]) > budget:
                break
            kept.insert(0, msg)
            acc += _estimate_tokens([msg])
        folded_count = len(rest) - len(kept)
        if folded_count <= 0:
            return history
        fold_note = ChatMessage(
            role="system",
            content=(
                f"[上下文压缩] 更早的 {folded_count} 条消息已折叠 (超出 token 预算 {budget})。"
                "仅保留最近对话以继续当前任务。"
            ),
        )
        return [sys_prompt, fold_note] + kept
...
def _estimate_tokens(messages: List[ChatMessage]) -> int:
    """粗略估算消息 token 数 (中文约 1.5 字符/token, 英文约 4 字符/token)。"""
    total = 0
    for m in messages:
        c = getattr(m, "content", "")
        if isinstance(c, list):
            for p in c:
                if isinstance(p, dict) and p.get("type") == "text":
                    total += _chars_to_tokens(p.get("text", ""))
        elif isinstance(c, str):
            total += _chars_to_tokens(c)
    return total
