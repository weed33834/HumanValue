...
class ComplianceService:
    ...
    async def _collect_access_log_evidence(self) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            audit_count = (
                await self.session.execute(select(func.count(AuditLog.id)))
            ).scalar() or 0
            audit_enabled = True
        except Exception as e:
        ...
        pii_working = False
        sample_input = (
            "手机 13800138000 邮箱 test@example.com 身份证 110101199003073888"
        )
    ...
    def _collect_change_evidence(self) -> Dict[str, Dict[str, Any]]:
        ...
        if not git_available:
            ...
            status_val = CONTROL_STATUS_WARNING
            msg = (
                f"最近 7 天无 Git 提交记录 (since={since_date}), 请确认变更流程是否正常"
            )
        else:
    ...
    def _collect_backup_evidence(self) -> Dict[str, Dict[str, Any]]:
        ...
        if backup_exists:
            ...
            backup_msg = f"备份目录 {backup_dir} 无备份文件, 请确认备份任务已配置"
    ...
    async def generate_report(
        ...
        *,
        ...
        compliance_score = (
            round((pass_count / assessable) * 100, 2) if assessable else 0.0
        )
