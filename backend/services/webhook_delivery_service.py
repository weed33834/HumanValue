...
3. **签名防重放**: Stripe 风格 ``X-HumanValue-Signature: t=<ts>,v1=<hex>``
...
1. 取 header ``X-HumanValue-Signature``, 形如 ``t=1754630400,v1=9f86d0...``;
...
def verify_signature(
    ...
    *,
    ...
    Args:
        secret: 订阅密钥。
        body: 收到的原始请求体字符串。
        signature_header: ``X-HumanValue-Signature`` 头原值。
    ...
    *,
    ...
    def _fire() -> None:
        try:
            ...
            logger.warning("无事件循环, 事件 %s 未能分发 (tenant=%s)", event, tenant_id)
            return
    ...
    try:
        ...
        logger.exception("挂载 after_commit 失败, 降级为立即分发 event=%s", event)
        _fire()
        return True
...
async def deliver(delivery_id: int) -> Dict[str, Any]:
    ...
    async with session_factory() as session:
        ...
        if await _is_url_blocked(url):
            ...
            await session.commit()
            logger.warning(
                "webhook 投递被 SSRF 防护拦截 delivery=%s url=%s", delivery_id, url
            )
            return {
                "delivery_id": delivery_id,
                "status": DELIVERY_STATUS_DEAD,
        ...
        headers = {
            ...
            "User-Agent": "HumanValue-Webhook/1.0",
            "X-HumanValue-Event": delivery.event,
            "X-HumanValue-Delivery": str(delivery.id),
            "X-HumanValue-Timestamp": str(timestamp),
            "X-HumanValue-Signature": build_signature(secret, timestamp, body),
        }
...
async def test_subscription(subscription_id: int) -> Dict[str, Any]:
    ...
    async with session_factory() as session:
        ...
        delivery = WebhookDelivery(
            ...
            payload={
                "message": "HumanValue webhook 连通性测试",
                "subscription_id": subscription.id,
