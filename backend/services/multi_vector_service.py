...
class MultiVectorSearchService:
        ...
        for granularity, collection_name in self._granularity_collections():
            ...
            try:
                if scope is None:
                    ...
                    count = sum(1 for m in metadatas if _chunk_tenant_allowed(m, scope))
    ...
    async def _index_granularity(
        ...
        for idx, (chunk, vec) in enumerate(zip(chunks, vectors)):
            ...
            content_hash = hashlib.sha256(chunk.encode("utf-8")).hexdigest()[:8]
    ...
    async def _delete_from_collection(
        ...
        try:
            ...
            result = await asyncio.to_thread(collection.get, where={"doc_id": doc_id})
