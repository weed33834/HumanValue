#!/usr/bin/env python3
"""HumanValue 全链路端到端冒烟测试 (E2E Smoke Test)
...
class E2ERunner:
    ...
    def _post_eval_payload(
        self, role: str = "manager", employee_id: str = "E1001", period: str = "2026-Q3"
    ):
    ...
    def phase_infra(self) -> None:
        ...
        def _ready() -> tuple[bool, str]:
            ...
            return (
                bool(checks.get("db")),
                f"db={checks.get('db')} redis={checks.get('redis')} provider={checks.get('provider')}",
            )
    ...
    def phase_auth(self) -> None:
        ...
        for role, email in ACCOUNTS.items():
            ...
            def _login(email=email, role=role) -> tuple[bool, str]:
                ...
                return (
                    data.get("role") == role,
                    f"role={data.get('role')} user_id={data.get('user_id')}",
                )
        ...
        def _me() -> tuple[bool, str]:
            r = self.client.get(
                f"{self.base}/api/v1/auth/me", headers=self._hdr("manager")
            )
            return (
                r.status_code == 200,
                f"status={r.status_code} name={r.json().get('name') if r.status_code==200 else ''}",
            )
        ...
        def _no_token() -> tuple[bool, str]:
            ...
            return (
                ...
                f"status={r.status_code}（未返回 200 即安全）",
            )
    ...
    def phase_evaluation(self) -> Optional[str]:
        ...
        def _poll() -> tuple[bool, str]:
            ...
            while time.time() < deadline:
                r = self.client.get(
                    f"{self.base}/api/v1/evaluations/jobs/{job_id}",
                    headers=self._hdr("manager"),
                )
                if r.status_code != 200:
        ...
        ok = self.check(
            "异步任务在 90s 内完成并产出 evaluation_id", "evaluation", _poll
        )
        ...
        def _get() -> tuple[bool, str]:
            r = self.client.get(
                f"{self.base}/api/v1/evaluations/{eval_id}",
                headers=self._hdr("manager"),
            )
            if r.status_code != 200:
        ...
        def _manager_view() -> tuple[bool, str]:
            r = self.client.get(
                f"{self.base}/api/v1/evaluations/{eval_id}/manager-view",
                headers=self._hdr("manager"),
            )
            return r.status_code == 200, f"status={r.status_code}"
        ...
        def _employee_view() -> tuple[bool, str]:
            r = self.client.get(
                f"{self.base}/api/v1/evaluations/{eval_id}/employee-view",
        ...
        def _evidence() -> tuple[bool, str]:
            r = self.client.get(
                f"{self.base}/api/v1/evaluations/{eval_id}/evidence",
                headers=self._hdr("manager"),
            )
            return r.status_code == 200, f"status={r.status_code}"
        ...
        def _audit() -> tuple[bool, str]:
            r = self.client.get(
                f"{self.base}/api/v1/evaluations/{eval_id}/audit-logs",
                headers=self._hdr("hr"),
            )
            return r.status_code == 200, f"status={r.status_code}"

        def _not_found() -> tuple[bool, str]:
            r = self.client.get(
                f"{self.base}/api/v1/evaluations/does-not-exist-xyz",
                headers=self._hdr("manager"),
            )
        ...
        def _job_has_eval_id() -> tuple[bool, str]:
            ...
            return (
                bool((r.json() or {}).get("evaluation_id")),
                "任务载荷顶层含 evaluation_id",
            )
    ...
    def phase_validation(self) -> None:
        ...
        def _empty_inputs() -> tuple[bool, str]:
            ...
            return (
                r.status_code in (200, 201, 202, 400, 422),
                f"status={r.status_code}（不应 5xx）",
            )
        ...
        def _oversized() -> tuple[bool, str]:
            r = self.client.post(
                ...
                json={
                    ...
                    "period": "2026-Q3",
                    "raw_inputs": [
                        {"type": "self_report", "content": "超长文本" * 50000}
                    ],
                },
            )
        ...
        def _injection() -> tuple[bool, str]:
            ...
            return (
                ...
                f"status={r.status_code}，注入后 users 表仍可用={probe.status_code==200}",
            )
        ...
        def _malformed_json() -> tuple[bool, str]:
            ...
            return (
                ...
                f"status={r.status_code}（应为 400/422）",
            )

        def _wrong_method() -> tuple[bool, str]:
            r = self.client.delete(
                f"{self.base}/api/v1/evaluations", headers=self._hdr("manager")
            )
    ...
    def phase_rbac(self) -> None:
        ...
        def _employee_cannot_admin() -> tuple[bool, str]:
            r = self.client.get(
                f"{self.base}/api/v1/admin/users", headers=self._hdr("employee")
            )
            return (
                r.status_code in (401, 403),
                f"status={r.status_code}（员工访问管理端应被拒）",
            )

        def _manager_cannot_admin() -> tuple[bool, str]:
            r = self.client.get(
                f"{self.base}/api/v1/admin/users", headers=self._hdr("manager")
            )
            return (
                r.status_code in (401, 403),
                f"status={r.status_code}（主管访问管理端应被拒）",
            )

        def _admin_can_admin() -> tuple[bool, str]:
            r = self.client.get(
                f"{self.base}/api/v1/admin/users", headers=self._hdr("admin")
            )
        ...
        def _employee_cannot_create_eval() -> tuple[bool, str]:
            r = self.client.post(
                ...
                json={
                    "employee_id": "E1002",
                    "period": "2026-Q3",
                    "raw_inputs": [{"type": "self_report", "content": "x"}],
                },
            )
            return (
                r.status_code in (401, 403, 429),
                f"status={r.status_code}（员工不应能为他人发起评估）",
            )
        ...
        def _manager_cross_team_blocked() -> tuple[bool, str]:
            ...
            r = self.client.post(
                ...
                json={
                    ...
                    "period": "2026-Q3",
                    "raw_inputs": [
                        {"type": "self_report", "content": "跨团队越权探测"}
                    ],
                },
            )
            return (
                r.status_code in (401, 403, 429),
                f"status={r.status_code}（跨团队写入应被拒）",
            )
        ...
        def _header_spoof() -> tuple[bool, str]:
            ...
            return (
                r.status_code in (401, 403),
                f"status={r.status_code}（Header 提权应无效）",
            )
    ...
    def phase_hitl(self, eval_id: Optional[str]) -> None:
            ...
            r = self.client.post(
                ...
                json={
                    ...
                    "raw_inputs": [
                        {
                            "type": "self_report",
                            "content": "季度交付 3 个核心模块，代码评审通过率 98%。",
                        }
                    ],
                },
            )
        ...
        if ok and thread_id:
            ...
            def _state_after_resume() -> tuple[bool, str]:
                ...
                if r.status_code == 200:
                    st = str(r.json())
                    return (
                        "awaiting_review" not in st,
                        f"status=200 已离开待审状态={'awaiting_review' not in st}",
                    )
                return False, f"status={r.status_code}"
        ...
        if eval_id:
            ...
            def _appeal() -> tuple[bool, str]:
                ...
                return (
                    r.status_code in (200, 201, 202, 400, 409),
    ...
    def phase_resilience(self) -> None:
                ...
                try:
                    r = c.post(
                        ...
                        json={
                            ...
                            "period": f"2026-CC{i}",
                            "raw_inputs": [
                                {"type": "self_report", "content": f"并发压测样本 {i}"}
                            ],
                        },
                    )
        ...
        def _rate_limit_behavior() -> tuple[bool, str]:
            ...
            return (
                ...
                f"30 次快速请求，状态码集合={sorted(set(codes))}",
            )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="HumanValue 全链路 E2E 冒烟测试")
    parser.add_argument("--base-url", default=DEFAULT_BASE_URL)
    parser.add_argument("--json", dest="json_out", help="导出 JSON 报告路径")
    args = parser.parse_args()

    print("=" * 72)
    print(f"HumanValue E2E Smoke Test → {args.base_url}")
    print("=" * 72)
