...
def cleanup_old_backups(
    backup_dir: Path, retain_days: int = DEFAULT_RETAIN_DAYS
) -> int:
    """清理过期备份文件, 返回删除的文件数"""
...
def schedule_restore_test(
    scheduler, day_of_week: str = "sun", hour: int = 4, minute: int = 0
):
    ...
    async def _run_restore_verification():
        """执行恢复验证(异步包装)"""
        try:
            from scripts.db_restore_test import (
                _run_sqlite_restore_test,
                _run_postgres_restore_test,
            )
