...
def upgrade() -> None:
    ...
    if not _has_table(inspector, "abac_policies"):
        op.create_table(
            ...
            sa.Column(
                "resource_id", sa.String(128), nullable=False, server_default="*"
            ),
            ...
            sa.CheckConstraint(
                "effect IN ('allow', 'deny')", name="ck_abac_policy_effect_valid"
            ),
            sa.CheckConstraint(
                "subject_type IN ('user', 'role', 'group')",
    ...
    if not _has_table(inspector, "compliance_controls"):
        op.create_table(
            ...
            sa.Column(
                "status", sa.String(16), nullable=False, server_default="not_applicable"
            ),
            sa.Column(
                "evidence", sa.JSON(), nullable=True, server_default=sa.text("'{}'")
            ),
            ...
            sa.UniqueConstraint(
                "tenant_id", "framework", "control_id", name="uix_compliance_control"
            ),
        )
        op.create_index(
            "ix_compliance_controls_tenant_id", "compliance_controls", ["tenant_id"]
        )
        op.create_index(
            "ix_compliance_controls_framework", "compliance_controls", ["framework"]
        )
        op.create_index(
            "ix_compliance_controls_control_id", "compliance_controls", ["control_id"]
        )
        op.create_index(
            "ix_compliance_controls_status", "compliance_controls", ["status"]
        )
        op.create_index(
            "ix_compliance_framework_status",
            "compliance_controls",
            ["framework", "status"],
        )
        op.create_index(
            "ix_compliance_tenant_framework",
            "compliance_controls",
            ["tenant_id", "framework"],
        )
    ...
    if not _has_table(inspector, "compliance_evidences"):
        op.create_table(
            ...
            sa.Column(
                "collector", sa.String(64), nullable=False, server_default="system"
            ),
        )
        op.create_index(
            "ix_compliance_evidences_tenant_id", "compliance_evidences", ["tenant_id"]
        )
        op.create_index(
            "ix_compliance_evidences_framework", "compliance_evidences", ["framework"]
        )
        op.create_index(
            "ix_compliance_evidences_control_id", "compliance_evidences", ["control_id"]
        )
        op.create_index(
            "ix_compliance_evidences_evidence_type",
            "compliance_evidences",
            ["evidence_type"],
        )
        op.create_index(
            "ix_compliance_ev_control_time",
            "compliance_evidences",
            ["control_id", "collected_at"],
        )
        op.create_index(
            "ix_compliance_ev_tenant_time",
            "compliance_evidences",
    ...
    if not _has_table(inspector, "user_groups"):
        op.create_table(
            ...
            sa.UniqueConstraint(
                "tenant_id", "group_id", name="uix_tenant_user_group_id"
            ),
        )
        ...
        op.create_index(
            "ix_user_group_tenant_name", "user_groups", ["tenant_id", "name"]
        )
    ...
    if not _has_table(inspector, "user_group_members"):
        ...
        op.create_index(
            "ix_user_group_members_group_id", "user_group_members", ["group_id"]
        )
        op.create_index(
            "ix_user_group_members_tenant_id", "user_group_members", ["tenant_id"]
        )
        op.create_index(
            "ix_user_group_member_tenant_group",
            "user_group_members",
            ["tenant_id", "group_id"],
        )
