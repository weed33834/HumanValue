...
def upgrade() -> None:
    ...
    if not _has_table(inspector, "trace_records"):
        op.create_table(
            ...
            sa.Column(
                "tenant_id", sa.String(64), nullable=True, server_default="default"
            ),
        ...
        op.create_index(
            "ix_trace_records_trace_id", "trace_records", ["trace_id"], unique=True
        )
        ...
        op.create_index(
            "ix_trace_records_tenant_kind", "trace_records", ["tenant_id", "kind"]
        )
        op.create_index(
            "ix_trace_records_tenant_status", "trace_records", ["tenant_id", "status"]
        )
    ...
    if not _has_table(inspector, "span_records"):
        ...
        op.create_index(
            "ix_span_records_parent_span_id", "span_records", ["parent_span_id"]
        )
        ...
        op.create_index(
            "ix_span_records_trace_started", "span_records", ["trace_id", "started_at"]
        )
        op.create_index(
            "ix_span_records_tenant_model", "span_records", ["tenant_id", "model"]
        )
