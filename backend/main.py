...
from api.admin.search_routes import (
    multi_vector_router as admin_multi_vector_router,
)  # noqa: E402
...
from api.admin.agents_crud_routes import (
    router as admin_agents_crud_router,
)  # noqa: E402

# 发布记录管理 (契约补齐: 记录列表/创建/更新 + 通用渠道发布)
from api.admin.publish_manage_routes import (
    router as admin_publish_manage_router,
)  # noqa: E402

# 契约补齐集合路由 (11 个跨模块补充端点, 必须最后挂载)
from api.admin.contract_supplement_routes import (
    router as admin_contract_supplement_router,
)  # noqa: E402
...
from api.admin.webhook_event_routes import (
    router as admin_webhook_event_router,
)  # noqa: E402
...
# 人才管理扩展：目标管理 / 行动项 / IDP / 1:1 / PIP
from api.talent_routes import router as talent_router  # noqa: E402

# 员工敬业度：脉搏调研 / 认可奖励 / 继任规划
from api.engagement_routes import (
    router as engagement_router,
    succession_router,
)  # noqa: E402

# 人才成长：技能矩阵 / 薪酬洞察 / 内部流动 / 团队健康 / 多期趋势
from api.growth_routes import router as growth_router  # noqa: E402

# Agent 高级能力 Admin API (Planner / Reflector / MCP Server / Browser)
from api.admin.agent_advanced_routes import (
    router as admin_agent_advanced_router,
)  # noqa: E402

# A2A 协议公开端点 (Agent Card + JSON-RPC)
from api.a2a_routes import router as a2a_router  # noqa: E402

# M14 企业治理 用户侧 (MFA / 公告 / 工单)
from api.enterprise_routes import router as enterprise_router  # noqa: E402

# M14 企业治理 管理侧 (登录风控 / 公告 / 工单管理)
from api.admin.enterprise_admin_routes import (
    router as admin_enterprise_router,
)  # noqa: E402

# M18 文本处理
from api.text_routes import router as text_router  # noqa: E402

# 人才价值引擎 (老板/管理者视角)
from api.talent_value_routes import router as talent_value_router  # noqa: E402

# M20 数据治理 / M23 LLM 缓存管理
from api.admin.data_governance_routes import (
    cache_router as admin_llm_cache_router,
)  # noqa: E402
from api.admin.data_governance_routes import (
    router as admin_data_governance_router,
)  # noqa: E402

# M29 容灾 / M28 数据管道 / M25 AI 安全
from api.admin.dr_routes import router as admin_dr_router  # noqa: E402
from api.admin.pipeline_routes import router as admin_pipeline_router  # noqa: E402
from api.admin.security_routes import router as admin_security_router  # noqa: E402
...
app = FastAPI(
    title="HumanValue",
    description="AI 驱动人才价值量化与成长 Agent 系统",
    version="2.8.5",
    lifespan=lifespan,
)
...
# Agent 高级能力 Admin API
app.include_router(admin_agent_advanced_router)
# A2A 协议公开端点
app.include_router(a2a_router, tags=["a2a"])
# M14 企业治理 用户侧
app.include_router(enterprise_router, tags=["enterprise"])
# M14 企业治理 管理侧
app.include_router(admin_enterprise_router)
# M18 文本处理
app.include_router(text_router)
# 人才价值引擎
app.include_router(talent_value_router)
# M20 数据治理 / M23 LLM 缓存管理
app.include_router(admin_data_governance_router)
app.include_router(admin_llm_cache_router)
# M29 容灾 / M28 数据管道 / M25 AI 安全
app.include_router(admin_dr_router)
