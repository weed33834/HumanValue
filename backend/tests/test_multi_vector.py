...
def mv_service(kb_store):
    ...
    return MultiVectorSearchService(kb_store=kb_store, collection_prefix="kb_mv_test")
...
async def test_index_document_creates_vectors(mv_service):
    ...
    assert (
        ...
        == result["paragraphs"] + result["sentences"] + result["keywords"]
    )
...
async def test_index_document_reindex_replaces_old(mv_service):
    ...
    assert (
        ...
        < stats_before["total_vectors"] + result["total_vectors"]
    )
...
async def test_search_exact_match_quality(mv_service):
    ...
    assert (
        len(matched) == 1
    ), f"精确匹配句子未出现在结果中: {[r['content'] for r in results]}"
