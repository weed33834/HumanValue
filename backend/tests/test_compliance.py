...
async def test_initialize_controls_creates_all(compliance_service, db_session):
    ...
    assert created == count_controls_by_framework(
        FRAMEWORK_SOC2
    ) + count_controls_by_framework(FRAMEWORK_ISO27001)
...
async def test_automated_check_config_audit(
    ...
    jwt_control = await compliance_service.get_control(
        "CC6.1", framework=FRAMEWORK_SOC2
    )
    ...
    demo_control = await compliance_service.get_control(
        "CC1.1", framework=FRAMEWORK_SOC2
    )
...
async def test_automated_check_config_audit_extension_https_debug(
    ...
    debug_control = await compliance_service.get_control(
        "CC7.1", framework=FRAMEWORK_SOC2
    )
    ...
    https_control = await compliance_service.get_control(
        "CC7.2", framework=FRAMEWORK_SOC2
    )
...
async def test_automated_check_access_log(
    ...
    # 审计日志启用 -> pass (表可查)
    audit_control = await compliance_service.get_control(
        "CC7.3", framework=FRAMEWORK_SOC2
    )
    ...
    pii_control = await compliance_service.get_control(
        "CC7.4", framework=FRAMEWORK_SOC2
    )
...
async def test_generate_report_json_framework_filter(
    ...
    assert all(c["framework"] == FRAMEWORK_ISO27001 for c in report["controls"])
...
async def test_manual_update_nonexistent_control_raises(compliance_service, db_session):
