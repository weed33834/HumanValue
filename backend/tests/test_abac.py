...
async def _seed_group(
    session_factory, group_id: str, tenant_id: str = DEFAULT_TENANT_ID
):
    """播种一个 UserGroup"""
    async with session_factory() as session:
...
async def _add_member(
    ...
    """播种组成员关系"""
    async with session_factory() as session:
        m = UserGroupMember(user_id=user_id, group_id=group_id, tenant_id=tenant_id)
        session.add(m)
        await session.commit()
...
class TestUserGroupsAPI:
    ...
    def test_employee_forbidden(self, api_client):
        ...
        resp = api_client.get("/api/v1/admin/user-groups", headers=_employee_headers())
        assert resp.status_code == 403
    ...
    def test_group_crud(self, api_client):
        ...
        # 列表
        resp = api_client.get("/api/v1/admin/user-groups", headers=_admin_headers())
        assert resp.status_code == 200
    ...
    def test_delete_group_cascades_policies_and_members(
        self, api_client, initialized_db
    ):
        """删除组 → 成员关联 + 组策略一并清理"""
        api_client.post(
