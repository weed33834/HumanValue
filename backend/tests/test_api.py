...
def test_auth_register_and_login(client, mock_app_state):
    ...
    register_payload = {
        "user_id": "E2001",
        "name": "测试员工",
        "email": "test-register@humanvalue.ai",
        "password": "test123456",
        "role": "employee",
        "department": "测试部",
    }
    ...
    resp = client.post(
        "/api/v1/auth/login",
        json={"email": "test-register@humanvalue.ai", "password": "test123456"},
    )
    assert resp.status_code == 200
    ...
    assert me["email"] == "test-register@humanvalue.ai"
...
def test_auth_login_wrong_password(client, mock_app_state):
    ...
    resp = client.post(
        "/api/v1/auth/login",
        json={"email": "wrong-pwd@humanvalue.ai", "password": "wrong123"},
    )
...
def test_auth_jwt_role_enforced(client, mock_app_state):
    ...
    token = client.post(
        "/api/v1/auth/login",
        json={"email": "role-test@humanvalue.ai", "password": "test123456"},
    ).json()["access_token"]
...
def _register_and_login(
    client, user_id="E3001", email="logout-test@humanvalue.ai"
) -> str:
...
def test_auth_logout_idempotent(client, mock_app_state):
    ...
    token = _register_and_login(
        client, user_id="E3002", email="idempotent@humanvalue.ai"
    )
...
def test_auth_refresh_after_logout_blocked(client, mock_app_state):
    ...
    token = _register_and_login(client, user_id="E3003", email="refresh@humanvalue.ai")
    ...
    resp = client.post(
        "/api/v1/auth/login",
        json={"email": "refresh@humanvalue.ai", "password": "test123456"},
    )
    assert resp.status_code == 200
...
def test_employee_cannot_access_other_evaluation(
    ...
    resp = client.get(
        f"/api/v1/evaluations/{created_evaluation_id}",
        headers={"x-user-role": "employee", "x-user-id": "E9999"},
    )
    assert resp.status_code == 403
