...
def test_load_returns_prompt_text(loader):
    ...
    assert "HumanValue" in text or "员工" in text
...
def test_version_extracted_from_header(loader):
    # v1.2 上线后，daily_evaluation.md 头部标注 v1.2
    assert loader.version("daily_evaluation") == "v1.2"


def test_current_version_constant_matches_header(loader):
    # PromptLoader.CURRENT_VERSION 与文件头部版本保持一致
    assert loader.CURRENT_VERSION == "v1.2"
...
def test_list_versions_returns_sorted(loader):
    # v1.2 归档后，versions/ 目录下含 v0.1、v0.2、v1.0、v1.1、v1.2 五个快照
...
def test_v1_2_archive_matches_current(loader):
    # 当前生效 Prompt 与 v1.2 快照内容一致（仅文件位置不同）
    current_text = loader.load("daily_evaluation")
    archived_text = loader.load_version("daily_evaluation", "v1.2")
...
def test_v1_2_audit_prompt_version_field(loader):
    # v1.2 Schema 中 audit.prompt_version 默认值标注为 v1.2
    text = loader.load("daily_evaluation")
    assert '"prompt_version": "v1.2"' in text
