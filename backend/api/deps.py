...
def get_mfa_service(session: AsyncSession = Depends(get_db)):
    from services.enterprise_service import MFAService

    return MFAService(session)


def get_login_guard_service(session: AsyncSession = Depends(get_db)):
    from services.enterprise_service import LoginGuardService

    return LoginGuardService(session)


def get_announcement_service(session: AsyncSession = Depends(get_db)):
    from services.enterprise_service import AnnouncementService

    return AnnouncementService(session)


def get_ticket_service(session: AsyncSession = Depends(get_db)):
    from services.enterprise_service import TicketService

    return TicketService(session)


def get_data_governance_service(session: AsyncSession = Depends(get_db)):
    from services.data_governance_service import DataGovernanceService

    return DataGovernanceService(session)


def get_dr_service(session: AsyncSession = Depends(get_db)):
    from services.dr_service import DRService

    return DRService(session)


def get_pipeline_service(session: AsyncSession = Depends(get_db)):
    from services.pipeline_service import PipelineService

    return PipelineService(session)


def get_security_service(session: AsyncSession = Depends(get_db)):
