...
from api.deps import get_audit_service, get_login_guard_service, get_mfa_service
...
class LoginRequest(BaseModel):
    ...
    # C.3 MFA: 已绑定 MFA 的用户登录时需携带 TOTP 验证码
    otp: Optional[str] = Field(default=None, max_length=16)
...
async def login(
    ...
    login_guard=Depends(get_login_guard_service),
    mfa_service=Depends(get_mfa_service),
):
    """邮箱 + 密码登录（含 M14 登录风控 / MFA），返回 JWT"""
    ip = get_client_ip(request)
    ua = (request.headers.get("user-agent") or "")[:250]

    # 1) 登录风控：锁定检查
    if await login_guard.is_locked(payload.email, ip):
        await session.commit()
        raise HTTPException(
            status_code=423, detail="登录尝试过多，账号已临时锁定，请稍后再试"
        )
    ...
    if not user or not user.password_hash:
        await login_guard.record_attempt(
            payload.email, False, ip, ua, reason="unknown_user"
        )
        await session.commit()
    ...
    if not verify_password(payload.password, user.password_hash):
        ...
        await login_guard.record_attempt(
            payload.email, False, ip, ua, reason="wrong_password"
        )
        await session.commit()
        raise HTTPException(status_code=401, detail="邮箱或密码错误")

    # 2) MFA 检查
    mfa_enabled = await mfa_service.is_enabled(user.user_id)
    if mfa_enabled:
        otp = (payload.otp or "").strip()
        if not otp:
            await login_guard.record_attempt(
                payload.email, True, ip, ua, reason="mfa_required"
            )
            await session.commit()
            raise HTTPException(
                status_code=403,
                detail={"message": "需要双因子验证", "mfa_required": True},
            )
        if not await mfa_service.verify(user.user_id, otp, persist=False):
            await login_guard.record_attempt(
                payload.email, False, ip, ua, reason="mfa_failed"
            )
            await session.commit()
    ...
    if getattr(settings, "mfa_enforce_admin", False) and user.role in (
        "admin",
        "platform_admin",
    ):
        if not mfa_enabled:
            raise HTTPException(
                status_code=403, detail="管理员账号必须启用双因子认证后才能登录"
            )
    ...
    await login_guard.record_attempt(payload.email, True, ip, ua, reason=None)
    await session.commit()
    return TokenResponse(
        access_token=token, user_id=user.user_id, name=user.name, role=role.value
    )
...
async def refresh_token(
    ...
    new_token = await create_access_token_async(
        user.user_id, role.value, name=user.name
    )
...
async def seed_demo_users(
    ...
    demo_accounts = [
        ...
        (
            "E1002",
            "李四（员工）",
            "employee2@humanvalue.ai",
            "employee",
            "研发部",
            "M002",
        ),
        ("M001", "王五（主管）", "manager@humanvalue.ai", "manager", "研发部", None),
        ("M002", "赵六（主管）", "manager2@humanvalue.ai", "manager", "研发部", None),
        ("HR001", "孙七（HR）", "hr@humanvalue.ai", "hr", "人力资源部", None),
        (
            "ADMIN001",
            "周八（管理员）",
            "admin@humanvalue.ai",
