...
class SubscriptionCreate(BaseModel):
    ...
    secret: Optional[str] = Field(
        ...
        min_length=16,
        max_length=128,
        description="签名密钥，缺省自动生成",
    )
    headers: Optional[Dict[str, str]] = Field(
        default=None, description="附加请求头（不允许覆盖 X-HumanValue-* 签名头）"
    )
...
async def list_subscriptions(
    ...
    tenant_id = get_current_tenant()
    base = select(WebhookSubscription).where(WebhookSubscription.tenant_id == tenant_id)
