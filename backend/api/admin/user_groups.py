...
class PolicyCreate(BaseModel):
    ...
    resource_id: str = Field(
        default=WILDCARD, max_length=128, description="资源 ID, * 通配"
    )
    action: str = Field(
        ..., min_length=1, max_length=128, description="动作如 evaluation:read"
    )
...
async def add_members(
    ...
    for uid in unique_user_ids:
        if uid in existing_set:
            continue
        member = UserGroupMember(user_id=uid, group_id=group_id, tenant_id=tenant_id)
        session.add(member)
        added.append(member)
    await session.commit()
    for m in added:
