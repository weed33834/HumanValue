...
def _filter_job_payload(job: Dict[str, Any], role: Role) -> Dict[str, Any]:
    ...
    filtered = {
        ...
        if k not in ("employee_view", "manager_view", "audit")
    }
...
async def get_pending_approvals(
    ...
    if page_size < 1 or page_size > 200:
        page_size = 50
    # H7：manager 仅看到直属下属的数据；HR/ADMIN/BOSS 不受限
    if role == Role.MANAGER:
        current_user_id = await get_current_user_id(request)
    ...
    result = await eval_service.list_evaluations(
        statuses=list(EvaluationStatus.PENDING_STATUSES), limit=200
    )
    pending = result["items"]
    if report_ids is not None:
        pending = [e for e in pending if e.employee_id in report_ids]
    total = len(pending)
    # 按创建时间倒序后分页
...
async def get_manager_dashboard(
    ...
    completed_items = (
        await eval_service.list_evaluations(
            status=EvaluationStatus.COMPLETED, limit=200
        )
    )["items"]
    if report_ids is not None:
        completed_items = [e for e in completed_items if e.employee_id in report_ids]
    completed_items.sort(key=lambda e: e.created_at, reverse=True)
    # 评估进行中（AI 正在处理）
    processing_items = (
        await eval_service.list_evaluations(status=EvaluationStatus.PROCESSING, limit=5)
    )["items"]
    if report_ids is not None:
        processing_items = [e for e in processing_items if e.employee_id in report_ids]
    return {
        "pending_count": len(completed_items),
        "pending": [
            {
                "evaluation_id": e.evaluation_id,
                "employee_id": e.employee_id,
...
async def get_hr_audit_queue(
    ...
    """HR 审计队列：待 HR 复核的评估（hr_audit 状态）"""
    processing = (
...
async def _evaluate_action(
    ...
    if action not in (
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="action 必须为 approve / reject / request_hr_review / request_manager_review",
        )
    evaluation = await eval_service.get_evaluation(evaluation_id)
    if not evaluation:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="评估不存在")
    actor_id = payload.get("actor_id") or await get_current_user_id(request)
    comment = _validate_text_field(payload.get("comment", ""), "comment")
    # H7 对齐：manager 仅能审批/驳回直属下属的评估，HR/ADMIN/BOSS 不受限。
    await assert_manager_team_access(
        eval_service, role, evaluation.employee_id, actor_id
    )
    approval_service = ApprovalService(session)
    # 仅 approve(通过) 记录审批人；reject/退回/复核 等动作不设 approver_id
    approver_id = actor_id if action == "approve" else None
    try:
...
async def approve_evaluation(
    ...
    """主管/HR 审批评估（普通 /evaluations 创建流）。

    驱动审批状态机流转：ai_drafted/manager_review → approved 等。
    payload 可带 action 指定具体动作；缺省为 approve。
    """
    action = payload.get("action") or "approve"
    return await _evaluate_action(
        evaluation_id,
        action,
        payload,
        request,
...
@router.post("/evaluations/{evaluation_id}/request-manager-review")
@rate_limit("30/minute")
...
async def appeal_evaluation(
    ...
    """员工对评估结果申诉（approved/rejected → manager_review）。

    payload: {"current_status": "...", "actor_id": "...", "comment": "..."}
    """
    evaluation = await eval_service.get_evaluation(evaluation_id)
    if not evaluation:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="评估不存在")
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    # 记录申诉内容为 feedback(type=appeal)，供员工反馈/申诉面板追踪
...
async def get_model_status(
    app_state: AppState = Depends(get_app_state),
    role: Role = Depends(require_role(Role.ADMIN, Role.BOSS)),
):
    """获取模型状态与推荐档位"""
