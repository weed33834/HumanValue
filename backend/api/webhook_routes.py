...
async def _handle_gitlab_event(
    ...
    if event_type == "Push Hook":
        ...
        logger.info("GitLab Issue: title=%s action=%s state=%s", title, action, state)
        # 转发 Issue 事件到事件总线
        if bus:
            await bus.publish(
            ...
            logger.info("GitLab Issue 已转发到事件总线 repo=%s title=%s", repo, title)
...
async def feishu_webhook(
    ...
    tenant_id = request.headers.get("X-Tenant-Id") or get_current_tenant()
