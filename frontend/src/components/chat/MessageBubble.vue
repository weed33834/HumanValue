...
// ---- 收藏 / 取消收藏（存 localStorage，简单可靠） ----
const FAV_KEY = 'humanvalue_fav_messages'
...
function toggleFav() {
  const id = props.message.id
  if (!id || String(id).startsWith('local-') || String(id).startsWith('pending-')) {
    ElMessage.warning('请等待消息保存后再收藏')
    return
  }
  try {
    let list = JSON.parse(localStorage.getItem(FAV_KEY) || '[]')
    if (isFav.value) {
...
<template>
  <div class="msg-bubble" :class="message.role">
    ...
    <el-dialog
      ...
      :title="feedbackRating === 'like' ? '点赞反馈' : '点踩反馈'"
      width="400px"
      append-to-body
    >
      <el-form label-position="top">
        <el-form-item
          :label="feedbackRating === 'like' ? '您觉得好在哪里？' : '您觉得哪里需要改进？'"
        >
          <el-input
