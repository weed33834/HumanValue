...
// 联网搜索开关：开启后发送时提示 Agent 调用 web_search 工具获取实时信息
...
function onSend() {
  ...
  // 开启联网搜索时，注入工具调用指令（后端 Agent 会调用 web_search_tool）
  if (webSearch.value) {
    content = `[联网搜索模式] 请优先使用 web_search 工具检索实时信息后再回答。\n${content}`
  }
...
<template>
  <div class="chat-input">
    ...
    <div class="input-row">
      ...
      <!-- 功能5: 联网搜索开关 -->
      <el-tooltip
        :content="webSearch ? '已开启联网搜索' : '开启联网搜索(实时检索)'"
        placement="top"
      >
        <button
          class="icon-action-btn web-search-btn"
