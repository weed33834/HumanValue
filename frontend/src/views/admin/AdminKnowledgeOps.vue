  ...
  <div class="admin-knowledge-ops av-fade-in-up">
    <div class="page-header mb-16">
      <div class="page-title">
        ...
        <span>{ $t('v.admin.AdminKnowledgeOps.5') }</span>
      </div>
    </div>
    ...
    <el-tabs v-model="activeTab" @tab-change="onTabChange">
      ...
      <el-tab-pane label="GraphRAG" name="graphrag">
        <div class="toolbar mb-16">
          <el-button :loading="grLoading" @click="loadGraphTasks"
            ><el-icon><RefreshLeft /></el-icon>{ $t('v.admin.AdminKnowledgeOps.6') }</el-button
          >
          <el-button type="primary" @click="openGraphSearch"
            ><el-icon><Search /></el-icon>{ $t('v.admin.AdminKnowledgeOps.4') }</el-button
          >
        </div>
        <el-card v-loading="grLoading">
          <el-table :data="graphTasks" stripe :empty-text="$t('v.admin.AdminKnowledgeOps.0')">
            <el-table-column prop="id" label="ID" width="70" align="center" />
            <el-table-column
              prop="name"
              :label="$t('v.admin.AdminKnowledgeOps.1')"
              min-width="150"
              show-overflow-tooltip
            />
            <el-table-column
              prop="kb_id"
              :label="$t('v.admin.AdminKnowledgeOps.2')"
              min-width="140"
              show-overflow-tooltip
            />
            <el-table-column :label="$t('v.admin.AdminKnowledgeOps.3')" width="120">
              <template #default="{ row }"
                ><el-tag size="small" :type="taskStatusType(row.status)">{{
                  taskStatusLabel(row.status)
                }}</el-tag></template
              >
            </el-table-column>
            <el-table-column label="实体/关系" width="130"
              ><template #default="{ row }"
                >{{ row.entity_count ?? 0 }} / {{ row.relation_count ?? 0 }}</template
              ></el-table-column
            >
            <el-table-column label="操作" width="220" fixed="right">
              <template #default="{ row }">
                <el-button size="small" link type="primary" @click="openEntitiesDialog(row)"
                  >实体</el-button
                >
                <el-button size="small" link @click="openRelationsDialog(row)">关系</el-button>
                <el-button size="small" link type="success" @click="openGraphSearch(row)"
                  >搜索</el-button
                >
              </template>
            </el-table-column>
      ...
      <el-tab-pane label="知识库同步" name="sync">
        <div class="toolbar mb-16">
          <el-button :loading="syncLoading" @click="loadSources"
        ...
        <el-card v-loading="syncLoading">
          <el-table :data="sources" stripe empty-text="暂无数据源">
            ...
            <el-table-column label="状态" width="120">
              <template #default="{ row }"
                ><el-tag size="small" :type="syncStatusType(row.status)">{{
      ...
      <el-tab-pane label="深度文档解析" name="parse">
        <div class="toolbar mb-16">
          <el-button :loading="parseLoading" @click="loadParseTasks"
            ><el-icon><RefreshLeft /></el-icon>刷新</el-button
          >
        </div>
            ...
            <el-table-column label="操作" width="180" fixed="right">
              <template #default="{ row }">
                <el-button
                  ...
                  >处理</el-button
                >
                <el-button size="small" link type="primary" @click="openParseResult(row)"
                  >结果</el-button
                >
              </template>
            </el-table-column>
      ...
      <el-tab-pane label="NL2SQL" name="nl2sql">
        <el-card class="mb-16">
          <div class="toolbar mb-16">
            ...
            <el-button :loading="translateLoading" type="primary" @click="handleTranslate"
              >翻译为 SQL</el-button
            >
            <el-button :loading="execLoading" type="success" @click="handleExecute"
              >执行 SQL</el-button
            >
          </div>
        ...
        <el-card class="mt-16">
          <div class="toolbar mb-16">
            ...
            <el-button :loading="schemaLoading" @click="loadSchemas"
              ><el-icon><RefreshLeft /></el-icon>刷新</el-button
            >
          </div>
          <el-table :data="schemas" stripe empty-text="暂无 Schema">
            ...
            <el-table-column label="表数量" width="100"
              ><template #default="{ row }">{{ row.table_count ?? 0 }}</template></el-table-column
            >
          </el-table>
        </el-card>
      </el-tab-pane>
      ...
      <el-tab-pane label="混合检索配置" name="search">
        <el-card v-loading="searchLoading">
          <div class="toolbar mb-16">
            ...
            <el-button :loading="searchLoading" @click="loadSearchConfig"
              ><el-icon><RefreshLeft /></el-icon>刷新</el-button
            >
            <el-button
              type="success"
              :loading="searchConfigSubmitting"
          ...
          <el-form label-position="top" v-loading="searchConfigSubmitting">
            <el-form-item :label="`向量检索权重: ${searchConfig.vector_weight}`">
              <el-slider
                v-model="searchConfig.vector_weight"
                :min="0"
                :max="1"
                :step="0.05"
                show-input
            ...
            <el-form-item label="Rerank 模型"
              ><el-input v-model="searchConfig.rerank_model" placeholder="bge-reranker-v2"
            /></el-form-item>
            <el-form-item label="启用 Rerank"
              ><el-switch v-model="searchConfig.enable_rerank"
            /></el-form-item>
          </el-form>
        </el-card>
        <el-card class="mt-16">
          <div class="toolbar mb-16">
            ...
            <el-button type="primary" :loading="testLoading" @click="handleSearchTest"
              >测试</el-button
            >
          </div>
          <el-form label-position="top">
            <el-form-item label="查询"
              ><el-input v-model="testQuery" placeholder="输入测试查询"
            /></el-form-item>
          </el-form>
    ...
    <!-- 实体查看 -->
    <el-dialog
      v-model="entitiesDialogVisible"
      :title="`实体 - ${entitiesTarget?.name || ''}`"
      width="720px"
    >
      <el-table
        :data="entities"
      ...
      <template #footer
        ><el-button @click="entitiesDialogVisible = false">关闭</el-button></template
      >
    </el-dialog>
    ...
    >
      ...
      <template #footer
        ><el-button @click="relationsDialogVisible = false">关闭</el-button></template
      >
    </el-dialog>
    ...
    <el-dialog v-model="searchDialogVisible" title="图谱搜索" width="560px">
      <el-form label-position="top" v-loading="searchSubmitting">
        <el-form-item label="查询"
          ><el-input v-model="graphQuery" placeholder="输入搜索内容"
        /></el-form-item>
      </el-form>
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="searchSubmitting" @click="handleGraphSearch"
          >搜索</el-button
        >
      </template>
    </el-dialog>
    ...
    <el-dialog
      ...
      :title="`同步日志 - ${syncLogsTarget?.name || ''}`"
      width="720px"
    >
      <el-table
        :data="syncLogs"
        v-loading="syncLogsLoading"
        stripe
        empty-text="暂无日志"
        max-height="440"
      >
        ...
        <el-table-column label="时间" width="180"
          ><template #default="{ row }">{{ formatTime(row.started_at) }}</template></el-table-column
        >
    ...
    <el-dialog
      ...
      :title="`解析结果 - ${parseResultTarget?.filename || ''}`"
      width="680px"
    >
...
async function loadGraphTasks() {
  ...
  try {
    const data = await graphRagApi.listTasks()
    graphTasks.value = data.items || []
  } catch (err) {
    ElMessage.error('加载 GraphRAG 任务失败: ' + (err.message || ''))
  } finally {
...
async function openEntitiesDialog(row) {
  ...
  entities.value = []
  try {
    const data = await graphRagApi.entities(row.id)
    entities.value = data.items || []
  } catch (err) {
    ElMessage.error('加载实体失败: ' + (err.message || ''))
  } finally {
...
async function openRelationsDialog(row) {
  ...
  relations.value = []
  try {
    const data = await graphRagApi.relations(row.id)
    relations.value = data.items || []
  } catch (err) {
    ElMessage.error('加载关系失败: ' + (err.message || ''))
  } finally {
...
async function handleGraphSearch() {
  ...
  try {
    searchResult.value = await graphRagApi.search({ query: graphQuery.value })
  } catch (err) {
...
async function loadSources() {
  ...
  try {
    const data = await kbSyncApi.listSources()
...
async function openSyncLogs(row) {
  ...
  syncLogs.value = []
  try {
    const data = await kbSyncApi.logs(row.id)
...
async function loadParseTasks() {
  ...
  try {
    const data = await docParsingApi.list()
    parseTasks.value = data.items || []
  } catch (err) {
    ElMessage.error('加载解析任务失败: ' + (err.message || ''))
  } finally {
...
async function handleProcess(row) {
  actingId.value = row.id
  try {
    await docParsingApi.process(row.id)
...
async function openParseResult(row) {
  ...
  try {
    ...
    parseResultText.value = '加载失败: ' + (err.message || '')
  }
}
...
async function loadSchemas() {
  ...
  try {
    const data = await nl2sqlApi.listSchemas()
...
async function handleTranslate() {
  ...
  try {
    const data = await nl2sqlApi.generate({
      question: nlQuestion.value,
      schema_id: nlSchema.value || undefined,
    })
...
async function handleExecute() {
  if (!nlSql.value.trim()) {
    ElMessage.warning('请先生成或输入 SQL')
    return
  }
  execLoading.value = true
  execResult.value = null
  try {
    const data = await nl2sqlApi.execute({
      sql: nlSql.value,
      schema_id: nlSchema.value || undefined,
    })
    execResult.value = data
  } catch (err) {
    execResult.value = '执行失败: ' + (err.message || '')
  } finally {
...
async function loadSearchConfig() {
  ...
  try {
    ...
    Object.assign(
      searchConfig,
      { vector_weight: 0.7, keyword_weight: 0.3, top_k: 10, rerank_model: '', enable_rerank: true },
      data || {},
    )
  } catch (err) {
    ElMessage.error('加载检索配置失败: ' + (err.message || ''))
  } finally {
...
async function handleSaveSearchConfig() {
  ...
  try {
    await searchAdminApi.updateConfig({ ...searchConfig })
    ElMessage.success('保存成功')
  } catch (err) {
    ElMessage.error('保存失败: ' + (err.message || ''))
  } finally {
    searchConfigSubmitting.value = false
  }
}
...
async function handleSearchTest() {
  ...
  try {
    testResult.value = await searchAdminApi.search({ query: testQuery.value })
  } catch (err) {
...
function taskStatusLabel(v) {
  return (
    { pending: '待处理', running: '处理中', completed: '已完成', failed: '失败' }[v] || v || '—'
  )
}
function syncStatusType(v) {
  return { idle: 'info', syncing: 'warning', success: 'success', failed: 'danger' }[v] || 'info'
}
function syncStatusLabel(v) {
  return { idle: '空闲', syncing: '同步中', success: '成功', failed: '失败' }[v] || v || '—'
}
