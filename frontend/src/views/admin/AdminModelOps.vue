  ...
  <div class="admin-model-ops av-fade-in-up">
    <div class="page-header mb-16">
      <div class="page-title">
        <el-icon><Cpu /></el-icon>
        <span>{ $t('v.admin.AdminModelOps.5') }</span>
      </div>
    </div>

    <el-tabs v-model="activeTab" @tab-change="onTabChange">
      <!-- ============ 容灾策略 ============ -->
      <el-tab-pane :label="$t('v.admin.AdminModelOps.0')" name="fallback">
        <div class="toolbar mb-16">
          <el-button :loading="fbLoading" @click="loadFallback"
            ><el-icon><RefreshLeft /></el-icon>{ $t('v.admin.AdminModelOps.6') }</el-button
          >
          <el-button type="primary" @click="openFallbackDialog"
            ><el-icon><Plus /></el-icon>{ $t('v.admin.AdminModelOps.4') }</el-button
          >
        </div>
        <el-card v-loading="fbLoading">
          <el-table :data="fallbackChains" stripe :empty-text="$t('v.admin.AdminModelOps.1')">
            <el-table-column prop="id" label="ID" width="70" align="center" />
            <el-table-column
              prop="name"
              :label="$t('v.admin.AdminModelOps.2')"
              min-width="140"
              show-overflow-tooltip
            />
            <el-table-column :label="$t('v.admin.AdminModelOps.3')" min-width="260">
            ...
            <el-table-column label="操作" width="180" fixed="right">
              <template #default="{ row }">
                <el-button
                  ...
                  @click="handleTestFallback(row)"
                  >测试</el-button
                >
                <el-button size="small" link type="danger" @click="handleDeleteFallback(row)"
                  >删除</el-button
                >
              </template>
            </el-table-column>
      ...
      <el-tab-pane label="负载均衡" name="lb">
        <div class="toolbar mb-16">
          <el-button :loading="lbLoading" @click="loadInstances"
        ...
        <el-card v-loading="lbLoading">
          <el-table :data="instances" stripe empty-text="暂无实例">
            ...
            <el-table-column label="健康" width="110">
              <template #default="{ row }">
                <el-tag size="small" :type="healthTagType(row.health)">{{
      ...
      <el-tab-pane label="API 健康监控" name="health">
        ...
        <div class="toolbar mb-16">
          ...
          <el-button :loading="sloLoading" @click="loadSlos"
            ><el-icon><RefreshLeft /></el-icon>刷新</el-button
          >
          <el-button type="primary" @click="openSloDialog"
            ><el-icon><Plus /></el-icon>新建 SLO</el-button
          >
        </div>
        <el-card v-loading="sloLoading">
          <el-table :data="slos" stripe empty-text="暂无 SLO">
            ...
            <el-table-column label="目标延迟" width="110"
              ><template #default="{ row }">{{
                row.target_latency_ms != null ? row.target_latency_ms + 'ms' : '—'
              }}</template></el-table-column
            >
            <el-table-column label="目标成功率" width="120"
              ><template #default="{ row }">{{
                row.target_success_rate != null
                  ? (row.target_success_rate * 100).toFixed(2) + '%'
                  : '—'
              }}</template></el-table-column
            >
            <el-table-column label="窗口" width="90"
              ><template #default="{ row }">{{
                row.window_minutes != null ? row.window_minutes + 'min' : '—'
              }}</template></el-table-column
            >
            ...
            <el-table-column label="操作" width="100" fixed="right">
              <template #default="{ row }">
                <el-button size="small" link type="danger" @click="handleDeleteSlo(row)"
                  >删除</el-button
                >
              </template>
            </el-table-column>
    ...
    <el-dialog v-model="fallbackDialogVisible" title="新建 Fallback 链" width="520px">
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="fbSubmitting" @click="handleSubmitFallback"
          >保存</el-button
        >
      </template>
    </el-dialog>
    ...
    <!-- LB 配置 -->
    <el-dialog
      v-model="lbConfigVisible"
      :title="`配置实例 - ${lbTarget?.name || ''}`"
      width="460px"
    >
      <el-form label-position="top" v-loading="lbConfigSubmitting">
        <el-form-item label="权重"
          ><el-input-number v-model="lbConfigForm.weight" :min="0" :max="100" style="width: 100%"
        /></el-form-item>
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="lbConfigSubmitting" @click="handleSubmitLbConfig"
          >保存</el-button
        >
      </template>
    </el-dialog>
    ...
    <el-dialog v-model="sloDialogVisible" title="新建 SLO" width="500px">
      <el-form
        ...
        :rules="sloRules"
        label-position="top"
        v-loading="sloSubmitting"
      >
        <el-form-item label="名称" prop="name"><el-input v-model="sloForm.name" /></el-form-item>
        <el-form-item label="端点" prop="endpoint"
          ><el-input v-model="sloForm.endpoint" placeholder="/v1/chat/completions"
        /></el-form-item>
        <el-form-item label="目标 (如 99.9 表示 99.9%)" prop="target"
          ><el-input-number
            v-model="sloForm.target"
            :min="0"
            :max="100"
            :precision="2"
            style="width: 100%"
        ...
        <el-form-item label="指标">
          <el-select v-model="sloForm.metric" style="width: 100%">
            <el-option label="可用率" value="availability" /><el-option
              label="P99 延迟"
              value="latency_p99"
...
async function handleSubmitFallback() {
  ...
  try {
    await fbFormRef.value.validate()
  } catch {
    return
  }
  const models = fbForm.modelsText
    .split('\n')
    .map((s) => s.trim())
    .filter(Boolean)
  if (!models.length) {
...
async function handleDeleteFallback(row) {
  try {
    await ElMessageBox.confirm(`确认删除容灾策略 "${row.name}"?`, '删除确认', { type: 'warning' })
  } catch {
    return
  }
  try {
...
async function handleSubmitLbConfig() {
  ...
  try {
    await modelLoadBalancerApi.updateConfig(lbTarget.value.id, {
      weight: lbConfigForm.weight,
      enabled: lbConfigForm.enabled,
    })
...
async function handleSubmitSlo() {
  ...
  try {
    await sloFormRef.value.validate()
  } catch {
    return
  }
...
async function handleDeleteSlo(row) {
  try {
    await ElMessageBox.confirm(`确认删除 SLO "${row.name}"?`, '删除确认', { type: 'warning' })
  } catch {
    return
  }
  try {
