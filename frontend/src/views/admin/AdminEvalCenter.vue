  ...
  <div class="admin-eval-center av-fade-in-up">
    <div class="page-header mb-16">
      <div class="page-title">
        ...
        <span>{ $t('v.admin.AdminEvalCenter.5') }</span>
      </div>
    </div>

    <el-tabs v-model="activeTab" @tab-change="onTabChange">
      <!-- ============ 数据集 ============ -->
      <el-tab-pane :label="$t('v.admin.AdminEvalCenter.0')" name="dataset">
        <div class="toolbar mb-16">
          <el-button :loading="dsLoading" @click="loadDatasets"
            ><el-icon><RefreshLeft /></el-icon>{ $t('v.admin.AdminEvalCenter.6') }</el-button
          >
          <el-button type="primary" @click="openDatasetDialog"
            ><el-icon><Plus /></el-icon>{ $t('v.admin.AdminEvalCenter.4') }</el-button
          >
        </div>
        <el-card v-loading="dsLoading">
          <el-table :data="datasets" stripe :empty-text="$t('v.admin.AdminEvalCenter.1')">
            <el-table-column prop="id" label="ID" width="70" align="center" />
            <el-table-column
              prop="name"
              :label="$t('v.admin.AdminEvalCenter.2')"
              min-width="160"
              show-overflow-tooltip
            />
            <el-table-column prop="type" :label="$t('v.admin.AdminEvalCenter.3')" width="120"
              ><template #default="{ row }"
                ><el-tag size="small">{{ row.type || '—' }}</el-tag></template
              ></el-table-column
            >
            <el-table-column label="条目数" width="100"
              ><template #default="{ row }">{{ row.item_count ?? 0 }}</template></el-table-column
            >
            ...
            <el-table-column label="操作" width="280" fixed="right">
              <template #default="{ row }">
                <el-button size="small" link type="primary" @click="openItemsDialog(row)"
                  >条目</el-button
                >
      ...
      <el-tab-pane label="LLM Judge" name="judge">
        <div class="toolbar mb-16">
          <el-button :loading="judgeLoading" @click="loadJudges"
            ><el-icon><RefreshLeft /></el-icon>刷新</el-button
          >
          <el-button type="primary" @click="openJudgeDialog"
            ><el-icon><VideoPlay /></el-icon>新建评测任务</el-button
          >
        </div>
        <el-card v-loading="judgeLoading">
          <el-table :data="judges" stripe empty-text="暂无评测任务">
            ...
            <el-table-column label="状态" width="120">
              <template #default="{ row }"
                ><el-tag size="small" :type="evalStatusType(row.status)">{{
            ...
            <el-table-column label="操作" width="180" fixed="right">
              <template #default="{ row }">
                <el-button
                  ...
                  @click="handleRunJudge(row)"
                  >运行</el-button
                >
                <el-button size="small" link type="primary" @click="openJudgeResultDialog(row)"
                  >结果</el-button
                >
              </template>
            </el-table-column>
      ...
      <el-tab-pane label="RAG 评测" name="rag">
        <div class="toolbar mb-16">
          <el-button :loading="ragLoading" @click="loadRags"
            ><el-icon><RefreshLeft /></el-icon>刷新</el-button
          >
          <el-button type="primary" @click="openRagDialog"
            ><el-icon><VideoPlay /></el-icon>新建评测任务</el-button
          >
        </div>
            ...
            <el-table-column label="操作" width="180" fixed="right">
              <template #default="{ row }">
                <el-button
                  ...
                  type="success"
                  :loading="actingId === row.id"
                  @click="handleRunRag(row)"
                  >运行</el-button
                >
                <el-button size="small" link type="primary" @click="openRagResultDialog(row)"
                  >结果</el-button
                >
              </template>
            </el-table-column>
      ...
      <el-tab-pane label="人工标注" name="annotation">
        <div class="toolbar mb-16">
          <el-button :loading="annoLoading" @click="loadAnnotations"
            ><el-icon><RefreshLeft /></el-icon>刷新</el-button
          >
          <el-button type="primary" @click="openAnnoDialog"
            ><el-icon><Plus /></el-icon>新建标注任务</el-button
          >
        </div>
          ...
          <el-table :data="annotations" stripe empty-text="暂无标注任务">
            ...
            <el-table-column label="操作" width="220" fixed="right">
              <template #default="{ row }">
                ...
                <el-button size="small" link type="primary" @click="openSubmitDialog(row)"
                  >标注</el-button
                >
                <el-button size="small" link type="success" @click="openAnnoStats(row)"
                  >统计</el-button
                >
              </template>
            </el-table-column>
    ...
    <el-dialog v-model="dsDialogVisible" title="新建数据集" width="480px">
      ...
      >
        ...
        <el-form-item label="类型"
          ><el-select v-model="dsForm.type" style="width: 100%"
            ><el-option label="LLM Judge" value="llm_judge" /><el-option
              label="RAG"
              value="rag" /><el-option label="标注" value="annotation" /></el-select
        ></el-form-item>
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="dsSubmitting" @click="handleSubmitDataset"
          >创建</el-button
        >
      </template>
    </el-dialog>
    ...
    <el-dialog
      ...
      :title="`导入条目 - ${importTarget?.name || ''}`"
      width="560px"
    >
      <el-alert type="info" :closable="false" show-icon class="mb-16"
        >粘贴 JSON 数组，每项含 input / expected 字段</el-alert
      >
    ...
    <el-dialog v-model="judgeDialogVisible" title="新建 LLM Judge 任务" width="500px">
      <el-form
        ...
        :rules="judgeRules"
        label-position="top"
        v-loading="judgeSubmitting"
      >
        <el-form-item label="任务名称" prop="name"
          ><el-input v-model="judgeForm.name"
        /></el-form-item>
        <el-form-item label="数据集" prop="dataset_id"
          ><el-select v-model="judgeForm.dataset_id" style="width: 100%"
            ><el-option
              v-for="d in datasets"
              :key="d.id"
              :label="d.name"
        ...
        ></el-form-item>
        <el-form-item label="Judge 模型" prop="judge_model"
          ><el-input v-model="judgeForm.judge_model" placeholder="gpt-4o"
        /></el-form-item>
        <el-form-item label="待评测模型"
          ><el-input v-model="judgeForm.target_model" placeholder="claude-3.5-sonnet"
        /></el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="judgeDialogVisible = false">取消</el-button>
        <el-button type="primary" :loading="judgeSubmitting" @click="handleSubmitJudge"
          >创建并运行</el-button
        >
      </template>
    </el-dialog>
    ...
    <el-dialog v-model="ragDialogVisible" title="新建 RAG 评测任务" width="500px">
      <el-form
        ...
        :rules="ragRules"
        label-position="top"
        v-loading="ragSubmitting"
      >
        <el-form-item label="任务名称" prop="name"
          ><el-input v-model="ragForm.name"
        /></el-form-item>
        <el-form-item label="数据集" prop="dataset_id"
          ><el-select v-model="ragForm.dataset_id" style="width: 100%"
            ><el-option
              v-for="d in datasets"
              :key="d.id"
              :label="d.name"
              :value="d.id" /></el-select
        ></el-form-item>
        <el-form-item label="检索器"
          ><el-input v-model="ragForm.retriever" placeholder="hybrid / vector / bm25"
        /></el-form-item>
        <el-form-item label="知识库 ID"><el-input v-model="ragForm.kb_id" /></el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="ragDialogVisible = false">取消</el-button>
        <el-button type="primary" :loading="ragSubmitting" @click="handleSubmitRag"
          >创建并运行</el-button
        >
      </template>
    </el-dialog>
    ...
    <el-dialog v-model="annoDialogVisible" title="新建标注任务" width="480px">
      <el-form
        ...
        :rules="annoRules"
        label-position="top"
        v-loading="annoSubmitting"
      >
        <el-form-item label="任务名称" prop="name"
          ><el-input v-model="annoForm.name"
        /></el-form-item>
        <el-form-item label="数据集" prop="dataset_id"
          ><el-select v-model="annoForm.dataset_id" style="width: 100%"
            ><el-option
              v-for="d in datasets"
              :key="d.id"
              :label="d.name"
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="annoSubmitting" @click="handleSubmitAnno"
          >创建</el-button
        >
      </template>
    </el-dialog>
    ...
    <el-dialog
      ...
      :title="`分配标注 - ${assignTarget?.name || ''}`"
      width="460px"
    >
      <el-form label-position="top" v-loading="assignSubmitting">
        <el-form-item label="标注人 (逗号分隔)"
          ><el-input v-model="assignForm.assignees" placeholder="user1, user2"
        /></el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog
      ...
      :title="`提交标注 - ${submitTarget?.name || ''}`"
      width="560px"
    >
      <el-form label-position="top" v-loading="submitSubmitting">
        <el-form-item label="条目 ID"
          ><el-input v-model.number="submitForm.item_id"
        /></el-form-item>
        <el-form-item label="标注结果"
          ><el-input v-model="submitForm.label" type="textarea" :rows="4"
        /></el-form-item>
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="submitSubmitting" @click="handleSubmitLabel"
          >提交</el-button
        >
      </template>
    </el-dialog>
    ...
    <!-- 标注统计 -->
    <el-dialog
      v-model="annoStatsVisible"
      :title="`标注统计 - ${annoStatsTarget?.name || ''}`"
      width="560px"
    >
...
async function loadDatasets() {
  ...
  try {
    ...
    ElMessage.error('加载数据集失败: ' + (err.message || ''))
  } finally {
...
function openDatasetDialog() {
  Object.assign(dsForm, { name: '', type: 'llm_judge', description: '' })
...
async function handleSubmitDataset() {
  ...
  try {
    await datasetApi.create({ ...dsForm })
...
async function loadItems() {
  ...
  try {
    const data = await datasetApi.listEntries(itemsTarget.value.id, {
    ...
    ElMessage.error('加载条目失败: ' + (err.message || ''))
  } finally {
...
async function handleImport() {
  ...
  try {
    ...
    ElMessage.error('不是合法的 JSON 数组')
    return
  }
  if (!Array.isArray(items)) {
    ElMessage.error('需为 JSON 数组')
    return
  }
  importSubmitting.value = true
  try {
    const data = await datasetApi.importEntries(importTarget.value.id, { items })
    ElMessage.success(`导入 ${data.imported ?? items.length} 条`)
...
async function loadJudges() {
  ...
  try {
    const data = await llmJudgeApi.list()
    judges.value = data.items || []
  } catch (err) {
    ElMessage.error('加载评测任务失败: ' + (err.message || ''))
  } finally {
...
function openJudgeDialog() {
  Object.assign(judgeForm, { name: '', dataset_id: null, judge_model: 'gpt-4o', target_model: '' })
...
async function handleSubmitJudge() {
  ...
  try {
    await judgeFormRef.value.validate()
  } catch {
    return
  }
  judgeSubmitting.value = true
  try {
    await llmJudgeApi.run({ ...judgeForm })
    ElMessage.success('任务已创建并运行')
...
async function handleRunJudge(row) {
  ...
  try {
    await llmJudgeApi.run({ id: row.id })
...
async function loadRags() {
  ...
  try {
    const data = await ragEvalApi.list()
    ragTasks.value = data.items || []
  } catch (err) {
    ElMessage.error('加载 RAG 任务失败: ' + (err.message || ''))
  } finally {
...
function openRagDialog() {
  Object.assign(ragForm, { name: '', dataset_id: null, retriever: 'hybrid', kb_id: '' })
...
async function handleSubmitRag() {
  ...
  try {
    await ragFormRef.value.validate()
  } catch {
    return
  }
  ragSubmitting.value = true
  try {
    await ragEvalApi.run({ ...ragForm })
    ElMessage.success('任务已创建并运行')
...
async function handleRunRag(row) {
  actingId.value = row.id
  try {
    await ragEvalApi.run({ id: row.id })
    ElMessage.success('已重新运行')
    await loadRags()
  } catch (err) {
    ElMessage.error('运行失败: ' + (err.message || ''))
...
async function openJudgeResultDialog(row) {
  ...
  try {
    ...
    resultText.value = '加载失败: ' + (err.message || '')
  }
}
...
async function loadAnnotations() {
  ...
  try {
    const data = await annotationApi.list()
    annotations.value = data.items || []
  } catch (err) {
    ElMessage.error('加载标注任务失败: ' + (err.message || ''))
  } finally {
...
function openAnnoDialog() {
  Object.assign(annoForm, { name: '', dataset_id: null })
...
async function handleSubmitAnno() {
  ...
  try {
    await annoFormRef.value.validate()
  } catch {
    return
  }
  annoSubmitting.value = true
  try {
    await annotationApi.create({ ...annoForm })
    ElMessage.success('创建成功')
...
async function handleAssign() {
  ...
  if (!assignees.length) {
    ElMessage.warning('请输入标注人')
    return
  }
  assignSubmitting.value = true
  try {
    await annotationApi.assign(assignTarget.value.id, { assignees })
...
function openSubmitDialog(row) {
  ...
  Object.assign(submitForm, { item_id: null, label: '', score: 0 })
  submitDialogVisible.value = true
}

async function handleSubmitLabel() {
  if (!submitForm.item_id) {
    ElMessage.warning('请输入条目 ID')
    return
  }
  submitSubmitting.value = true
  try {
    await annotationApi.annotate(submitTarget.value.id, {
      item_id: submitForm.item_id,
...
async function openAnnoStats(row) {
  ...
  try {
    annoStats.value = await annotationApi.stats(row.id)
  } catch (err) {
    ElMessage.error('加载统计失败: ' + (err.message || ''))
  } finally {
    annoStatsLoading.value = false
  }
}

// ====== 工具 ======
function evalStatusType(v) {
  return (
    { pending: 'info', running: 'warning', completed: 'success', failed: 'danger' }[v] || 'info'
  )
}
function evalStatusLabel(v) {
  return (
    { pending: '待运行', running: '运行中', completed: '已完成', failed: '失败' }[v] || v || '—'
  )
}
