  ...
  <div class="admin-export">
    ...
    <el-alert type="info" :closable="false" show-icon class="mb-16">
      <template #title>
        ...
        每个模块可选择导出格式 (CSV / Excel / JSON) 与日期范围，导出文件将自动下载。 该模块仅 admin
        / hr 角色可访问。
      </template>
    </el-alert>
    ...
    <div class="export-grid">
      <el-card v-for="mod in modules" :key="mod.key" class="export-card" shadow="hover">
        <template #header>
        ...
        <div class="card-footer">
          <el-button type="primary" :loading="state[mod.key].loading" @click="handleExport(mod)">
            <el-icon><Download /></el-icon>
    ...
    <el-card v-if="!exporting" class="tip-card mt-16" shadow="never">
      <el-empty
        description="选择上方任意模块并点击导出按钮，文件将自动下载至浏览器默认下载目录"
        :image-size="80"
      />
    </el-card>
  </div>
