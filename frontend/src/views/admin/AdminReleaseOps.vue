  ...
  <div class="admin-release-ops av-fade-in-up">
    <div class="page-header mb-16">
      <div class="page-title">
        ...
        <span>{ $t('v.admin.AdminReleaseOps.5') }</span>
      </div>
    </div>

    <el-tabs v-model="activeTab" @tab-change="onTabChange">
      <!-- ============ Agent 版本 ============ -->
      <el-tab-pane :label="$t('v.admin.AdminReleaseOps.0')" name="version">
        <div class="toolbar mb-16">
          <el-select
            v-model="currentAgentId"
            :placeholder="$t('v.admin.AdminReleaseOps.1')"
            style="width: 240px"
          ...
          <el-button :loading="verLoading" @click="loadVersions"
            ><el-icon><RefreshLeft /></el-icon>{ $t('v.admin.AdminReleaseOps.6') }</el-button
          >
          <el-button type="primary" :disabled="!currentAgentId" @click="openVersionDialog"
            ><el-icon><Upload /></el-icon>{ $t('v.admin.AdminReleaseOps.4') }</el-button
          >
        </div>
        <el-card v-loading="verLoading">
          <el-table :data="versions" stripe :empty-text="$t('v.admin.AdminReleaseOps.2')">
            <el-table-column
              prop="version_number"
              :label="$t('v.admin.AdminReleaseOps.3')"
              width="100"
              align="center"
            >
            ...
            <el-table-column label="温度" width="90" align="center"
              ><template #default="{ row }">{{
                (row.temperature ?? 0) / 100
              }}</template></el-table-column
            >
            <el-table-column label="状态" width="110">
              <template #default="{ row }"
                ><el-tag size="small" :type="verStatusType(row.status)">{{
                  verStatusLabel(row.status)
                }}</el-tag></template
              >
            </el-table-column>
            ...
            <el-table-column label="操作" width="260" fixed="right">
              <template #default="{ row }">
                <el-button size="small" link type="success" @click="openCompareDialog(row)"
                  >对比</el-button
                >
                <el-button
        ...
        <el-card v-loading="pubLoading">
          <el-table :data="publishConfigs" stripe empty-text="暂无发布记录">
            ...
            <el-table-column prop="channel" label="渠道" width="110">
              <template #default="{ row }"
                ><el-tag size="small">{{ row.channel }}</el-tag></template
              >
            </el-table-column>
            <el-table-column prop="version_id" label="版本 ID" width="100" align="center" />
            <el-table-column label="状态" width="110">
              <template #default="{ row }"
                ><el-tag size="small" :type="pubStatusType(row.status)">{{
                  pubStatusLabel(row.status)
                }}</el-tag></template
              >
            </el-table-column>
            <el-table-column label="发布时间" width="180"
      ...
      <el-tab-pane label="灰度发布" name="gray">
        <div class="toolbar mb-16">
          <el-button :loading="grayLoading" @click="loadGray"
            ><el-icon><RefreshLeft /></el-icon>刷新</el-button
          >
          <el-button type="primary" @click="openGrayDialog"
            ><el-icon><Plus /></el-icon>新建灰度任务</el-button
          >
        </div>
        <el-card v-loading="grayLoading">
          <el-table :data="grayTasks" stripe empty-text="暂无灰度任务">
            ...
            <el-table-column prop="release_type" label="类型" width="110">
              <template #default="{ row }"
                ><el-tag size="small">{{ row.release_type || 'canary' }}</el-tag></template
              >
            </el-table-column>
            <el-table-column label="灰度比例" width="110"
              ><template #default="{ row }">{{
                row.traffic_percentage != null ? row.traffic_percentage + '%' : '—'
              }}</template></el-table-column
            >
            <el-table-column label="状态" width="110">
              <template #default="{ row }"
                ><el-tag size="small" :type="grayStatusType(row.status)">{{
                  grayStatusLabel(row.status)
                }}</el-tag></template
              >
            </el-table-column>
            <el-table-column label="操作" width="330" fixed="right">
              <template #default="{ row }">
                <el-button size="small" link type="info" @click="openGrayStats(row)"
                  >统计</el-button
                >
                <el-button
                  ...
                  type="success"
                  :loading="actingId === row.id"
                  @click="handleGrayAction(row, 'start')"
                  >启动</el-button
                >
                <el-button
                  ...
                  type="warning"
                  :loading="actingId === row.id"
                  @click="handleGrayAction(row, 'pause')"
                  >暂停</el-button
                >
                <el-button
                ...
                <el-button
                  ...
                  type="primary"
                  :loading="actingId === row.id"
                  @click="handleGrayAction(row, 'complete')"
                  >完成</el-button
                >
                <el-button
                  v-if="row.status !== 'rolled_back' && row.status !== 'completed'"
                  size="small"
                  link
                  type="danger"
                  :loading="actingId === row.id"
                  @click="handleGrayAction(row, 'rollback')"
                  >回滚</el-button
                >
              </template>
            </el-table-column>
      ...
      <el-tab-pane label="环境管理" name="env">
        <div class="toolbar mb-16">
          <el-button :loading="envLoading" @click="loadEnvs"
            ><el-icon><RefreshLeft /></el-icon>刷新</el-button
          >
        </div>
        <el-card v-loading="envLoading">
          <el-table :data="environments" stripe empty-text="暂无环境">
            ...
            <el-table-column label="状态" width="110">
              <template #default="{ row }"
                ><el-tag size="small" :type="envStatusType(row.status)">{{
      ...
      <el-form ref="verFormRef" :model="verForm" label-position="top" v-loading="verSubmitting">
        ...
        <el-form-item label="变更日志"
          ><el-input
            ...
            type="textarea"
            :rows="3"
            placeholder="本次版本改动说明"
        /></el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="versionDialogVisible = false">取消</el-button>
        <el-button type="primary" :loading="verSubmitting" @click="handleSubmitVersion"
          >创建版本</el-button
        >
      </template>
    </el-dialog>
    ...
    <el-dialog v-model="publishDialogVisible" title="新建发布记录" width="520px">
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="pubSubmitting" @click="handleSubmitPublish"
          >创建</el-button
        >
      </template>
    </el-dialog>
    ...
    <el-dialog v-model="grayDialogVisible" title="新建灰度任务" width="520px">
      <el-form
        ...
        :rules="grayRules"
        label-position="top"
        v-loading="graySubmitting"
      >
        <el-form-item label="任务名称" prop="name"
          ><el-input v-model="grayForm.name"
        /></el-form-item>
        <el-form-item label="Agent" prop="agent_id">
        ...
        <el-form-item :label="`灰度比例: ${grayForm.traffic_percentage}%`"
          ><el-slider
            v-model="grayForm.traffic_percentage"
            :min="0"
            :max="100"
            :step="5"
            show-input
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="graySubmitting" @click="handleSubmitGray"
          >创建</el-button
        >
      </template>
    </el-dialog>
        ...
        <template v-if="grayStats">
          <el-descriptions :column="2" border size="small">
            ...
            <el-descriptions-item label="状态">{{
              grayStatusLabel(grayStats.status)
            }}</el-descriptions-item>
            <el-descriptions-item label="灰度流量"
              >{{ grayStats.traffic?.gray_percentage }}%</el-descriptions-item
            >
            <el-descriptions-item label="基线流量"
              >{{ grayStats.traffic?.baseline_percentage }}%</el-descriptions-item
            >
            <el-descriptions-item label="发布类型">{{
              grayStats.progress?.release_type
            }}</el-descriptions-item>
            <el-descriptions-item label="完成度">{{
              grayStats.progress?.percent_complete != null
                ? grayStats.progress.percent_complete + '%'
                : '—'
            }}</el-descriptions-item>
            <el-descriptions-item label="启动时间">{{
              formatTime(grayStats.timeline?.started_at)
            }}</el-descriptions-item>
            <el-descriptions-item label="运行时长">{{
              grayStats.timeline?.running_seconds != null
                ? grayStats.timeline.running_seconds + 's'
                : '—'
            }}</el-descriptions-item>
          </el-descriptions>
        </template>
    ...
    >
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="deploySubmitting" @click="handleDeployEnv"
          >部署</el-button
        >
      </template>
    </el-dialog>
...
async function loadAgents() {
  ...
  try {
    ...
    ElMessage.error('加载 Agent 列表失败: ' + (err.message || ''))
  } finally {
...
async function loadVersions() {
  ...
  try {
    ...
    ElMessage.error('加载版本失败: ' + (err.message || ''))
  } finally {
...
async function handleSubmitVersion() {
  ...
  try {
    ...
    verSubmitting.value = false
  }
}

async function handlePublishVersion(row) {
  try {
    await ElMessageBox.confirm(`确认发布版本 v${row.version_number}?`, '发布确认', {
      type: 'warning',
    })
  } catch {
    return
...
async function handleRollback(row) {
  try {
    await ElMessageBox.confirm(`确认回滚到版本 v${row.version_number}?`, '回滚确认', {
      type: 'warning',
    })
  } catch {
    return
...
async function handleCompare() {
  if (!compareForm.from || !compareForm.to) {
    ElMessage.warning('请选择两个版本')
    return
  }
  compareLoading.value = true
  try {
    const data = await agentVersionApi.compare(
...
async function loadPublish() {
  ...
  try {
    ...
    publishConfigs.value = data.items || []
  } catch (err) {
    ElMessage.error('加载发布记录失败: ' + (err.message || ''))
  } finally {
...
async function handleOffline(row) {
  try {
    await ElMessageBox.confirm(
      `确认下线 ${agentName(row.agent_id)} 的 ${row.channel} 渠道?`,
      '下线确认',
      { type: 'warning' },
    )
  } catch {
  ...
  actingId.value = row.id
  try {
    await publishApi.undeploy(row.agent_id, row.channel)
    ElMessage.success('下线成功')
    await loadPublish()
  } catch (err) {
    ElMessage.error('下线失败: ' + (err.message || ''))
  } finally {
    actingId.value = null
  }
}
...
async function loadGray() {
  ...
  try {
    const data = await grayReleaseApi.list()
    grayTasks.value = data.items || []
  } catch (err) {
    ElMessage.error('加载灰度任务失败: ' + (err.message || ''))
  } finally {
...
async function handleSubmitGray() {
  ...
  try {
    await grayFormRef.value.validate()
  } catch {
    return
  }
  graySubmitting.value = true
  try {
    await grayReleaseApi.create({ ...grayForm })
    ElMessage.success('创建成功')
...
async function openGrayStats(row) {
  ...
  try {
    grayStats.value = await grayReleaseApi.stats(row.id)
  } catch (err) {
    ElMessage.error('加载统计失败: ' + (err.message || ''))
  } finally {
    grayStatsLoading.value = false
  }
}
...
async function handleGrayAction(row, action) {
  ...
  if (action === 'rollback') {
    try {
      await ElMessageBox.confirm(`确认回滚灰度任务 "${row.name}"?`, '回滚确认', { type: 'warning' })
    } catch {
      return
    }
  }
  actingId.value = row.id
  try {
    await grayReleaseApi[action](row.id)
    ElMessage.success(`${labels[action]}成功`)
    await loadGray()
  } catch (err) {
    ElMessage.error(`${labels[action]}失败: ` + (err.message || ''))
  } finally {
    actingId.value = null
  }
}
...
async function loadEnvs() {
  ...
  try {
    ...
    ElMessage.error('加载环境失败: ' + (err.message || ''))
  } finally {
...
async function handleDeployEnv() {
  ...
  try {
    await environmentApi.deploy(deployTarget.value.id, { version: deployForm.version })
    ElMessage.success('部署成功')
...
async function handleUndeploy(row) {
  try {
    await ElMessageBox.confirm(`确认从环境 "${row.name}" 卸载?`, '卸载确认', { type: 'warning' })
  } catch {
    return
  }
  actingId.value = row.id
  try {
    await environmentApi.undeploy(row.id)
...
// ====== 工具函数 ======
function verStatusType(v) {
  return { published: 'success', archived: 'warning', draft: 'info' }[v] || 'info'
}
function verStatusLabel(v) {
  return { published: '已发布', archived: '已归档', draft: '草稿' }[v] || v || '—'
}
function pubStatusType(v) {
  return { published: 'success', pending: 'warning', failed: 'danger' }[v] || 'info'
}
function pubStatusLabel(v) {
  return { published: '已发布', pending: '待发布', failed: '失败' }[v] || v || '—'
}
function grayStatusType(v) {
  return (
    {
      draft: 'info',
      active: 'success',
...
function envStatusType(v) {
  return { deployed: 'success', idle: 'info', failed: 'danger' }[v] || 'info'
}
function envStatusLabel(v) {
  return { deployed: '已部署', idle: '空闲', failed: '失败' }[v] || v || '—'
}
