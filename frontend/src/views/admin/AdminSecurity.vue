  ...
  <div class="admin-security av-fade-in-up">
    <div class="page-header mb-16">
      <div class="page-title">
        ...
        <span>{ $t('v.admin.AdminSecurity.10') }</span>
      </div>
    </div>

    <el-tabs v-model="activeTab" @tab-change="onTabChange">
      <!-- ============ 敏感词管理 ============ -->
      <el-tab-pane :label="$t('v.admin.AdminSecurity.0')" name="sensitive">
        <div class="toolbar mb-16">
          <el-input
            v-model="swFilter.keyword"
            :placeholder="$t('v.admin.AdminSecurity.1')"
            clearable
            style="width: 220px"
            @change="loadWords"
          />
          <el-select
            v-model="swFilter.status"
            :placeholder="$t('v.admin.AdminSecurity.2')"
            clearable
            style="width: 160px"
            @change="loadWords"
          >
            <el-option :label="$t('v.admin.AdminSecurity.3')" value="pending" />
            <el-option :label="$t('v.admin.AdminSecurity.4')" value="approved" />
            <el-option :label="$t('v.admin.AdminSecurity.5')" value="rejected" />
          </el-select>
          <el-button :loading="swLoading" @click="loadWords"
            ><el-icon><RefreshLeft /></el-icon>{ $t('v.admin.AdminSecurity.12') }</el-button
          >
          <el-button type="primary" @click="openWordDialog"
            ><el-icon><Plus /></el-icon>{ $t('v.admin.AdminSecurity.13') }</el-button
          >
          <el-button type="warning" @click="importDialogVisible = true"
            ><el-icon><Upload /></el-icon>{ $t('v.admin.AdminSecurity.11') }</el-button
          >
        </div>
        <el-card v-loading="swLoading">
          <el-table :data="words" stripe :empty-text="$t('v.admin.AdminSecurity.6')">
            <el-table-column prop="id" label="ID" width="70" align="center" />
            <el-table-column
              prop="term"
              :label="$t('v.admin.AdminSecurity.7')"
              min-width="180"
              show-overflow-tooltip
            />
            <el-table-column prop="category" :label="$t('v.admin.AdminSecurity.8')" width="140" />
            <el-table-column :label="$t('v.admin.AdminSecurity.9')" width="120">
              <template #default="{ row }">
                <el-tag size="small" :type="reviewTagType(row.status)">{{
            ...
            <el-table-column label="操作" width="240" fixed="right">
              <template #default="{ row }">
                ...
                <el-button
                  ...
                  >拒绝</el-button
                >
                <el-button size="small" link type="danger" @click="handleDeleteWord(row)"
                  >删除</el-button
                >
              </template>
            </el-table-column>
      ...
      <el-tab-pane label="SSO 配置" name="sso">
        <div class="toolbar mb-16">
          <el-button :loading="ssoLoading" @click="loadSso"
            ><el-icon><RefreshLeft /></el-icon>刷新</el-button
          >
          <el-button type="primary" @click="openSsoDialog"
            ><el-icon><Plus /></el-icon>新增 SSO</el-button
          >
          <el-button type="success" @click="openLdapDialog"
            ><el-icon><Connection /></el-icon>LDAP 登录测试</el-button
          >
        </div>
        <el-card v-loading="ssoLoading">
          <el-table :data="ssoConfigs" stripe empty-text="暂无 SSO 配置">
            ...
            <el-table-column prop="provider_type" label="协议" width="120">
              <template #default="{ row }"
                ><el-tag size="small">{{ row.provider_type || '—' }}</el-tag></template
              >
            </el-table-column>
            ...
            <el-table-column label="操作" width="160" fixed="right">
              <template #default="{ row }">
                ...
                <el-button size="small" link type="danger" @click="handleDeleteSso(row)"
                  >删除</el-button
                >
              </template>
            </el-table-column>
    ...
    <el-dialog v-model="wordDialogVisible" title="新增敏感词" width="460px">
      <el-form
        ...
        :rules="wordRules"
        label-position="top"
        v-loading="wordSubmitting"
      >
        <el-form-item label="敏感词" prop="term"><el-input v-model="wordForm.term" /></el-form-item>
        <el-form-item label="分类"
          ><el-input v-model="wordForm.category" placeholder="如 政治/涉黄/广告"
        /></el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="wordDialogVisible = false">取消</el-button>
        <el-button type="primary" :loading="wordSubmitting" @click="handleSubmitWord"
          >保存</el-button
        >
      </template>
    </el-dialog>
    ...
    <el-dialog
      ...
      :title="ssoIsEdit ? '编辑 SSO 配置' : '新增 SSO 配置'"
      width="560px"
      @closed="resetSsoForm"
    >
      <el-form
        ...
        :rules="ssoRules"
        label-position="top"
        v-loading="ssoSubmitting"
      >
        <el-form-item label="提供商名称" prop="provider_name"
          ><el-input v-model="ssoForm.provider_name" placeholder="如 企业微信/飞书/Okta"
        /></el-form-item>
        ...
        <el-form-item
          :label="
            ssoForm.provider_type === 'ldap' ? 'LDAP 服务地址 (server_url)' : 'Issuer / 服务地址'
          "
        >
          <el-input
            v-model="ssoForm.issuer"
            :placeholder="
              ssoForm.provider_type === 'ldap' ? 'ldap://host:389' : 'https://sso.example.com'
            "
          />
        </el-form-item>
        <el-form-item label="Client ID"><el-input v-model="ssoForm.client_id" /></el-form-item>
        <el-form-item label="Client Secret / Bind Password"
          ><el-input v-model="ssoForm.client_secret" type="password" show-password
        /></el-form-item>
      ...
      <el-form label-position="top" v-loading="ldapSubmitting">
        <el-form-item label="选择 LDAP 配置" required>
          <el-select v-model="ldapForm.config_id" style="width: 100%" placeholder="选择 LDAP 配置">
            <el-option
              v-for="c in ldapConfigs"
              :key="c.id"
              :label="`#${c.id} ${c.provider_name}`"
              :value="c.id"
            />
          </el-select>
        </el-form-item>
        <el-form-item label="用户 DN / 用户名"
          ><el-input v-model="ldapForm.username" placeholder="uid=test,ou=users,dc=example,dc=com"
        /></el-form-item>
        <el-form-item label="密码"
          ><el-input v-model="ldapForm.password" type="password" show-password
        /></el-form-item>
      </el-form>
      <div v-if="ldapResult" class="ldap-result">
        ...
        <el-descriptions :column="1" border size="small">
          <el-descriptions-item label="状态"
            ><el-tag :type="ldapResult.success ? 'success' : 'danger'">{{
              ldapResult.success ? '登录成功' : '登录失败'
...
async function handleDeleteWord(row) {
  try {
    await ElMessageBox.confirm(`确认删除敏感词 "${row.term}"?`, '删除确认', { type: 'warning' })
  } catch {
    return
  }
  try {
...
async function handleImport() {
  const terms = importText.value
    .split('\n')
    .map((s) => s.trim())
    .filter((s) => s && !s.startsWith('#'))
  if (!terms.length) {
    ElMessage.warning('未检测到有效敏感词')
    return
  }
...
async function handleDeleteSso(row) {
  try {
    await ElMessageBox.confirm(`确认删除 SSO 配置 "${row.provider_name}"?`, '删除确认', {
      type: 'warning',
    })
  } catch {
    return
...
async function handleLdapTest() {
  if (!ldapForm.config_id) {
    ElMessage.warning('请先选择 LDAP 配置')
    return
  }
  if (!ldapForm.username || !ldapForm.password) {
    ElMessage.warning('请填写用户名和密码')
    return
  }
  ldapSubmitting.value = true
  try {
    const res = await ssoApi.ldapLogin(ldapForm.config_id, {
...
function reviewTagType(v) {
  return { pending: 'warning', approved: 'success', rejected: 'danger' }[v] || 'info'
}
