      ...
      <el-table :data="pagedSkills" stripe empty-text="暂无技能，点击「AI 生成」或「导入」开始">
        <el-table-column label="显示名称" min-width="180">
          <template #default="{ row }">
            <div class="skill-name-cell">
              <el-link type="primary" @click="openEditDialog(row)">{{
        ...
        <el-table-column label="所需工具" min-width="180">
          <template #default="{ row }">
            <template v-if="row.required_tools && row.required_tools.length">
              ...
              <el-tag v-if="row.required_tools.length > 3" size="small" type="info">
        ...
        <el-table-column label="状态" width="180">
          <template #default="{ row }">
            ...
            <el-tag v-if="row.is_public" size="small" type="success" class="status-tag">
              公开
            </el-tag>
            <el-tag v-if="row.is_active" size="small" type="primary" class="status-tag">
              激活
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="280" fixed="right">
          <template #default="{ row }">
            ...
            <el-button v-if="!row.is_builtin" size="small" link @click="openEditDialog(row)">
    ...
    >
      <el-form ref="skillFormRef" :model="skillForm" :rules="skillRules" label-position="top">
        ...
        <el-form-item :label="`温度 (Temperature): ${skillForm.temperature / 100}`">
          <el-slider v-model="skillForm.temperature" :min="0" :max="100" :step="5" show-stops />
          <span class="field-hint">
      ...
      <div v-if="generatedSkill" class="generate-preview">
        ...
        <el-descriptions :column="2" border size="small" class="mt-16">
          ...
          <el-descriptions-item label="版本">{{
            generatedSkill.version || '1.0'
          }}</el-descriptions-item>
          <el-descriptions-item label="模型层级">{{
            generatedSkill.model_tier || '-'
          }}</el-descriptions-item>
          <el-descriptions-item label="所需工具">
            ...
            <span
              v-if="!(generatedSkill.required_tools && generatedSkill.required_tools.length)"
              class="muted"
              >无</span
            >
          </el-descriptions-item>
      ...
      <div v-if="executeResult" class="execute-result">
        ...
        <div v-if="executeResult.token_usage" class="token-usage">
          <el-tag size="small" type="info"
            >Prompt: {{ executeResult.token_usage.prompt || 0 }}</el-tag
          >
          <el-tag size="small" type="info"
            >Completion: {{ executeResult.token_usage.completion || 0 }}</el-tag
          >
          <el-tag size="small" type="success"
            >Total: {{ executeResult.token_usage.total || 0 }}</el-tag
          >
        </div>
      </div>
    </el-dialog>
