  ...
  <div class="admin-quota-budget av-fade-in-up">
    <div class="page-header mb-16">
      <div class="page-title">
        ...
        <span>{ $t('v.admin.AdminQuotaBudget.4') }</span>
      </div>
    </div>

    <el-tabs v-model="activeTab" @tab-change="onTabChange">
      <!-- ============ 配额管理 ============ -->
      <el-tab-pane :label="$t('v.admin.AdminQuotaBudget.0')" name="quota">
        <div class="toolbar mb-16">
          <el-select v-model="usageDays" style="width: 160px" @change="loadQuotas">
            <el-option :value="7" :label="$t('v.admin.AdminQuotaBudget.1')" />
            <el-option :value="30" :label="$t('v.admin.AdminQuotaBudget.2')" />
            <el-option :value="90" :label="$t('v.admin.AdminQuotaBudget.3')" />
          </el-select>
        ...
        <el-card v-loading="quotaLoading" class="mb-16">
          <template #header>
            <span>{ $t('v.admin.AdminQuotaBudget.5') }</span>
            <el-tag
              v-if="quota"
              size="small"
              :type="quota.enabled ? 'success' : 'info'"
              class="ml-8"
            >
          ...
          <el-descriptions v-if="quota" :column="2" border>
            ...
            <el-descriptions-item label="配额重置时间">{{
              formatTime(quota.quota_reset_at)
            }}</el-descriptions-item>
            <el-descriptions-item label="今日请求数">
              <div class="usage-cell">
                ...
                <span class="usage-text">
                  {{ formatNum(quota.current_requests_today) }} /
                  {{ formatNum(quota.max_requests_per_day) }}
                </span>
              </div>
            </el-descriptions-item>
            <el-descriptions-item label="今日 Token 数">
              <div class="usage-cell">
                ...
                />
                <span class="usage-text">
                  {{ formatNum(quota.current_tokens_today) }} /
                  {{ formatNum(quota.max_tokens_per_day) }}
                </span>
              </div>
            </el-descriptions-item>
            <el-descriptions-item label="最大 API Key 数">{{
              formatNum(quota.max_api_keys)
            }}</el-descriptions-item>
            <el-descriptions-item label="更新时间">{{
              formatTime(quota.updated_at)
            }}</el-descriptions-item>
          </el-descriptions>
        ...
        <el-card v-loading="quotaLoading">
          ...
          <el-row v-if="usageSummary" :gutter="16" class="mb-16">
            ...
            <el-col :span="8">
              <el-statistic
                title="总成本 (USD)"
                :value="usageSummary.total_cost || 0"
                :precision="4"
              />
            </el-col>
          </el-row>
          ...
          <el-table :data="budgets" stripe empty-text="暂无预算记录">
            ...
            <el-table-column label="预算额度" width="130">
              <template #default="{ row }"
                >{{ formatNum(row.amount) }} {{ row.currency || '' }}</template
              >
            </el-table-column>
            <el-table-column label="已用 / 剩余" min-width="220">
              <template #default="{ row }">
                <div class="usage-cell">
                  ...
                  />
                  <span class="usage-text"
                    >{{ formatNum(row.used) }} / {{ formatNum(row.remaining) }}</span
                  >
                </div>
              </template>
            </el-table-column>
            <el-table-column label="预警阈值" width="120">
              <template #default="{ row }">
                <el-tag
                  size="small"
                  :type="budgetPercent(row) >= (row.alert_threshold || 0) ? 'danger' : 'info'"
                >
            ...
            <el-table-column label="状态" width="110">
              <template #default="{ row }">
                <el-tag size="small" :type="budgetStatusType(row)">{{
            ...
            <el-table-column label="操作" width="160" fixed="right">
              <template #default="{ row }">
                ...
                <el-button size="small" link type="danger" @click="handleDeleteBudget(row)"
                  >删除</el-button
                >
              </template>
            </el-table-column>
    ...
    >
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="budgetSubmitting" @click="handleSubmitBudget"
          >保存</el-button
        >
      </template>
    </el-dialog>
