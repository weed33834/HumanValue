  ...
  <div class="admin-scheduler av-fade-in-up">
    <div class="page-header mb-16">
      <div class="page-title">
        ...
        <span>{ $t('v.admin.AdminScheduler.3') }</span>
      </div>
    ...
    <el-card v-loading="loading">
      <el-table :data="tasks" stripe :empty-text="$t('v.admin.AdminScheduler.0')">
        <el-table-column prop="id" label="ID" width="70" align="center" />
        <el-table-column
          prop="name"
          :label="$t('v.admin.AdminScheduler.1')"
          min-width="160"
          show-overflow-tooltip
        />
        <el-table-column :label="$t('v.admin.AdminScheduler.2')" width="160">
          <template #default="{ row }"
            ><code class="mono">{{ row.cron }}</code></template
          >
        </el-table-column>
        ...
        <el-table-column label="上次结果" width="110">
          <template #default="{ row }">
            <el-tag v-if="row.last_status" size="small" :type="lastStatusType(row.last_status)">{{
              row.last_status
            }}</el-tag>
            <span v-else class="muted">—</span>
          </template>
...
function openDialog(row = null) {
  ...
  if (row) {
    Object.assign(form, {
      ...
      params: row.params ? JSON.stringify(row.params, null, 2) : '',
      enabled: !!row.enabled,
    })
  } else {
    resetForm()
...
function lastStatusType(v) {
  return (
    { success: 'success', failed: 'danger', running: 'warning', timeout: 'danger' }[v] || 'info'
  )
}
