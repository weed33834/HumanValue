  ...
  <div class="admin-billing av-fade-in-up">
    <div class="page-header mb-16">
      <div class="page-title">
        ...
        <span>{ $t('v.admin.AdminBilling.3') }</span>
      </div>
      <div class="toolbar-actions">
        <el-date-picker
          ...
          type="monthrange"
          range-separator="至"
          start-:placeholder="$t('v.admin.AdminBilling.0')"
          end-:placeholder="$t('v.admin.AdminBilling.1')"
          ...
          <div class="summary-body">
            ...
            <div class="summary-meta">
              ...
              <div class="summary-value">
                ...
                }}<span class="summary-unit">{{ summary.currency || 'CNY' }}</span>
              </div>
            </div>
          </div>
        </el-card>
    ...
    <el-card class="mb-16" v-loading="trendLoading">
      <template #header
        ><span class="section-title"
          ><el-icon><TrendCharts /></el-icon> 按月费用趋势</span
        ></template
      >
    ...
    <el-card class="mb-16" v-loading="byUserLoading">
      <template #header
        ><span class="section-title"
          ><el-icon><User /></el-icon> 按用户费用</span
        ></template
      >
      <el-table :data="byUser" stripe empty-text="暂无数据">
        ...
        <el-table-column label="请求数" width="120"
          ><template #default="{ row }">{{
            formatNum(row.request_count)
          }}</template></el-table-column
        >
        <el-table-column label="Token 用量" width="140"
          ><template #default="{ row }">{{ formatNum(row.token_count) }}</template></el-table-column
        >
        <el-table-column label="费用" width="140" align="right">
          <template #default="{ row }"
            ><span class="money-text"
              >{{ formatMoney(row.cost) }} {{ summary.currency || 'CNY' }}</span
            ></template
          >
        </el-table-column>
        <el-table-column label="占比" width="180">
          <template #default="{ row }">
            <el-progress
              :percentage="costPercent(row.cost)"
              :stroke-width="12"
              :text-inside="true"
            />
          </template>
      ...
      <div class="pagination-wrapper">
        <el-pagination
          ...
          @size-change="loadByUser"
          @current-change="loadByUser"
        />
      </div>
    </el-card>
    ...
    <el-card v-loading="byEndpointLoading">
      <template #header
        ><span class="section-title"
          ><el-icon><Connection /></el-icon> 按端点费用</span
        ></template
      >
      ...
      <div class="pagination-wrapper">
        <el-pagination
          ...
          :page-sizes="[10, 20, 50]"
          layout="total, sizes, prev, pager, next, jumper"
          @size-change="loadByEndpoint"
          @current-change="loadByEndpoint"
        />
      </div>
    </el-card>
  </div>
...
async function loadSummary() {
  ...
  try {
    summary.value = (await billingApi.summary(dateParams())) || {}
  } catch (err) {
    ElMessage.error('加载账单汇总失败: ' + (err.message || ''))
  } finally {
...
async function loadByUser() {
  ...
  try {
    const data = await billingApi.byUser({
      ...dateParams(),
      page: userPage.value,
...
async function loadByEndpoint() {
  ...
  try {
    const data = await billingApi.byEndpoint({
      ...dateParams(),
      page: endpointPage.value,
      page_size: endpointPageSize.value,
    })
