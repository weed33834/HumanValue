  ...
  <div class="admin-agent-templates av-fade-in-up">
    <div class="page-header mb-16">
      <div class="page-title">
        <el-icon><Box /></el-icon>
        <span>{ $t('v.admin.AdminAgentTemplates.10') }</span>
      </div>
      <div class="toolbar-actions">
        <el-input
          v-model="keyword"
          :placeholder="$t('v.admin.AdminAgentTemplates.0')"
          clearable
          style="width: 200px"
          @change="loadTemplates"
        />
        <el-button :loading="loading" @click="loadTemplates"
          ><el-icon><RefreshLeft /></el-icon>{ $t('v.admin.AdminAgentTemplates.14') }</el-button
        >
        <el-button type="primary" @click="openDialog()"
          ><el-icon><Plus /></el-icon>{ $t('v.admin.AdminAgentTemplates.12') }</el-button
        >
      </div>
    </div>
    ...
    <div v-loading="loading" class="template-grid">
      <el-empty
        ...
        :description="$t('v.admin.AdminAgentTemplates.1')"
      />
      <el-card v-for="tpl in templates" :key="tpl.id" class="template-card" shadow="hover">
        <div class="card-header">
          <div class="card-avatar" :style="{ background: avatarColor(tpl) }">
        ...
        <div class="card-stats">
          <span
            ><el-icon><Download /></el-icon> {{ tpl.use_count ?? 0 }} 次实例化</span
          >
          <span
            ><el-icon><Star /></el-icon> {{ tpl.rating ?? 0 }}</span
          >
        </div>
        <div class="card-actions">
          <el-button
            size="small"
            type="primary"
            :loading="instantiatingId === tpl.id"
            @click="handleInstantiate(tpl)"
          >
            <el-icon><MagicStick /></el-icon>实例化
          </el-button>
          <el-button size="small" @click="openDialog(tpl)"
            >{ $t('v.admin.AdminAgentTemplates.18') }</el-button
          >
          <el-button size="small" type="danger" link @click="handleDelete(tpl)"
            >{ $t('v.admin.AdminAgentTemplates.16') }</el-button
          >
        </div>
      </el-card>
    </div>
    ...
    >
      ...
      >
        <el-form-item :label="$t('v.admin.AdminAgentTemplates.2')" prop="name">
          <el-input v-model="form.name" :placeholder="$t('v.admin.AdminAgentTemplates.3')" />
        </el-form-item>
        <el-form-item :label="$t('v.admin.AdminAgentTemplates.4')" prop="category">
          <el-select v-model="form.category" style="width: 100%">
            <el-option v-for="c in categories" :key="c.value" :label="c.label" :value="c.value" />
          </el-select>
        </el-form-item>
        <el-form-item :label="$t('v.admin.AdminAgentTemplates.5')" prop="description">
        ...
        <el-form-item label="System Prompt" prop="system_prompt">
          <el-input
            ...
            type="textarea"
            :rows="5"
            :placeholder="$t('v.admin.AdminAgentTemplates.6')"
          />
        </el-form-item>
        <el-form-item :label="$t('v.admin.AdminAgentTemplates.7')">
          <el-input v-model="form.model" placeholder="gpt-4o" />
        </el-form-item>
        <el-form-item :label="$t('v.admin.AdminAgentTemplates.8')">
          <el-input v-model="form.tools" placeholder="search, calculator" />
        </el-form-item>
        <el-form-item :label="$t('v.admin.AdminAgentTemplates.9')">
          <el-switch v-model="form.is_public" />
          <span class="form-hint">{ $t('v.admin.AdminAgentTemplates.11') }</span>
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="dialogVisible = false"
          >{ $t('v.admin.AdminAgentTemplates.15') }</el-button
        >
        <el-button type="primary" :loading="submitting" @click="handleSubmit"
          >{ $t('v.admin.AdminAgentTemplates.17') }</el-button
        >
      </template>
    </el-dialog>
    ...
    <el-dialog
      ...
      :title="`实例化模板 - ${instantiateTarget?.name || ''}`"
      width="480px"
    >
      <el-form label-position="top" v-loading="instantiating">
        <el-form-item label="实例名称"
          ><el-input
            ...
            :placeholder="`${instantiateTarget?.name} 实例`"
        /></el-form-item>
        <el-form-item label="所属租户 (可选)"
          ><el-input v-model="instantiateForm.tenant_id"
        /></el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="instantiateDialogVisible = false">取消</el-button>
        <el-button type="primary" :loading="instantiating" @click="confirmInstantiate"
          >实例化</el-button
        >
      </template>
    </el-dialog>
...
async function loadStats() {
  try {
    stats.value = (await agentTemplateApi.stats()) || {}
  } catch (err) {
...
async function handleDelete(row) {
  ...
  try {
    await agentTemplateApi.delete(row.id)
...
async function confirmInstantiate() {
  ...
  try {
    await agentTemplateApi.instantiate(instantiateTarget.value.id, {
      name: instantiateForm.name,
      tenant_id: instantiateForm.tenant_id || null,
    })
