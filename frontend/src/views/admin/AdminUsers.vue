  ...
  <div class="admin-users">
    ...
    <el-alert type="info" :closable="false" show-icon class="mb-16">
      <template #title>
        ...
        支持按角色与部门筛选、关键字搜索，以及 CSV 批量导入（含导入结果反馈）。 该模块仅 admin
        角色可访问。
      </template>
    </el-alert>
    ...
    >
      ...
      >
        ...
        <el-form-item label="姓名" prop="name">
          <el-input v-model="form.name" placeholder="请输入姓名" maxlength="64" show-word-limit />
        </el-form-item>
...
async function handleToggleDisable(row) {
  ...
  try {
    await ElMessageBox.confirm(`确认${action}用户 "${row.name || row.user_id}"?`, `${action}确认`, {
      type: 'warning',
