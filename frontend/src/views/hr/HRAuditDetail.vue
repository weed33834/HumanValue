<template>
  ...
  <div class="hr-audit-detail">
    <el-page-header @back="goBack" :title="$t('v.hr.HRAuditDetail.0')" />
    ...
    <el-skeleton
      ...
      animated
      role="status"
      aria-:label="$t('v.hr.HRAuditDetail.1')"
      class="detail-skeleton"
    />

    <el-empty v-else-if="!evaluation" :description="$t('v.hr.HRAuditDetail.2')" />
    ...
    <el-card v-else v-loading="loading" :aria-busy="loading" class="detail-card">
      <template #header>
        <div class="card-header">
          ...
          <el-tag :type="statusType" :aria-:label="$t('v.hr.HRAuditDetail.3')">
            {{ evaluation.status }}
          </el-tag>
        </div>
      </template>
