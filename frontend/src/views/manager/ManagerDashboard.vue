<template>
  <div class="manager-dashboard av-page">
    <el-row :gutter="20">
      <el-col :span="24">
        <el-card>
          <template #header>
            <div class="card-header">
              <span>{{ $t('pages2.dashboard.pending', { n: pendingCount }) }}</span>
              <el-button size="small" :loading="loading" @click="loadData">{{
                $t('pages2.dashboard.refresh')
              }}</el-button>
            </div>
          </template>
          <el-table
            v-loading="loading"
            :aria-busy="loading"
            :data="pendingItems"
            style="width: 100%"
            :empty-text="$t('pages2.dashboard.empty')"
          >
            <el-table-column prop="employee_id" :label="$t('pages2.dashboard.colEmp')" />
            <el-table-column prop="period" :label="$t('pages2.dashboard.colPeriod')" />
            <el-table-column
              prop="overall_score"
              :label="$t('pages2.dashboard.colScore')"
              sortable
            />
            <el-table-column prop="status" :label="$t('pages2.dashboard.colStatus')" />
            <el-table-column label="操作" width="180">
              <template #default="{ row }">
                <el-button size="small" type="primary" @click="viewDetail(row)">{{
                  $t('pages2.dashboard.view')
                }}</el-button>
              </template>
            </el-table-column>
          </el-table>
        </el-card>
      </el-col>
    </el-row>

    <el-row :gutter="20" class="mt-20 av-stagger">
      <el-col :span="12">
        <el-card>
          <template #header>
            <span>{{ $t('pages2.dashboard.inProgress') }}</span>
          </template>
          <el-table :data="recentEvaluations" style="width: 100%">
            <el-table-column prop="employee_id" :label="$t('pages2.dashboard.colEmp')" />
            <el-table-column prop="period" :label="$t('pages2.dashboard.colPeriod')" />
            <el-table-column prop="overall_score" label="得分" />
          </el-table>
        </el-card>
      </el-col>
      <el-col :span="12">
        <el-card>
          <template #header>
            <span>{{ $t('pages2.dashboard.riskDist') }}</span>
          </template>
          <div class="risk-summary">
            <!-- 无障碍：风险等级已由 title 文字传达，数值颜色加深以满足对比度要求 -->
            <el-statistic
              :title="$t('pages2.dashboard.high')"
              :value="riskStats.high"
              value-style="color: var(--el-color-danger)"
            />
            <el-statistic
              :title="$t('pages2.dashboard.medium')"
              :value="riskStats.medium"
              value-style="color: var(--el-color-warning)"
            />
            <el-statistic
              :title="$t('pages2.dashboard.low')"
              :value="riskStats.low"
              value-style="color: var(--el-color-success)"
            />
          </div>
        </el-card>
      </el-col>
    </el-row>
  </div>
</template>

<script setup>
import { ref, onMounted } from 'vue'
import { useRouter } from 'vue-router'
import { ElMessage } from 'element-plus'
import { managerApi } from '@/api/client'

const router = useRouter()

const loading = ref(false)
const pendingCount = ref(0)
const pendingItems = ref([])
const recentEvaluations = ref([])
const riskStats = ref({ high: 0, medium: 0, low: 0 })

async function loadData() {
  loading.value = true
  try {
    const data = await managerApi.dashboard()
    pendingCount.value = data.pending_count || 0
    pendingItems.value = data.pending || []
    recentEvaluations.value = data.recent_evaluations || data.recent_approved || []
    // 从后端返回的 risk_stats 或基于 pending 数据计算风险分布
    if (data.risk_stats) {
      riskStats.value = data.risk_stats
    } else if (pendingItems.value.length > 0) {
      const stats = { high: 0, medium: 0, low: 0 }
      for (const evalItem of pendingItems.value) {
        const score = evalItem.overall_score ?? 100
        const riskFlags = evalItem.manager_view?.risk_flags || []
        const hasCritical = riskFlags.some((r) => r.level === 'critical')
        if (hasCritical || score < 60) stats.high++
        else if (score < 75) stats.medium++
        else stats.low++
      }
      riskStats.value = stats
    }
  } catch (err) {
    console.error('加载工作台失败:', err)
    ElMessage.error('加载工作台失败')
  } finally {
    loading.value = false
  }
}

function viewDetail(row) {
  router.push(`/boss/evaluation/${row.evaluation_id}`)
}

onMounted(loadData)
</script>

<style scoped>
.mt-20 {
  margin-top: var(--av-gap-lg);
}
.risk-summary {
  display: flex;
  justify-content: space-around;
  flex-wrap: wrap;
  gap: var(--av-gap-md);
  padding: var(--av-gap-md) 0;
}
.card-header {
  display: flex;
  justify-content: space-between;
  align-items: center;
  gap: var(--av-gap-md);
  font-weight: 600;
}
/* 卡片头部统一弹性布局与间距 */
.manager-dashboard :deep(.el-card__header) {
  display: flex;
  align-items: center;
  gap: var(--av-gap-md);
}
/* 交互元素过渡效果 */
.manager-dashboard :deep(.el-card) {
  transition:
    box-shadow var(--av-transition-base) var(--av-ease-smooth),
    transform var(--av-transition-base) var(--av-ease-smooth);
}
.manager-dashboard :deep(.el-statistic) {
  transition: transform var(--av-transition-base) var(--av-ease-smooth);
}

/* 响应式：移动端适配 */
@media (max-width: 768px) {
  .manager-dashboard :deep(.el-col) {
    max-width: 100%;
    flex: 0 0 100%;
  }
  .mt-20 {
    margin-top: var(--av-gap-md);
  }
  .risk-summary {
    padding: var(--av-gap-sm) 0;
  }
  .manager-dashboard :deep(.el-table) {
    width: 100%;
    overflow-x: auto;
  }
}
</style>
