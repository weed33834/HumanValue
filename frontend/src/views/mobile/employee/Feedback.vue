  ...
  <div class="m-page">
    <div class="m-card">
      ...
      <div class="m-field">
        ...
        <div class="m-list" style="margin: 0">
          <div
            ...
            :class="{
              'm-list-item--active': selected && selected.evaluation_id === ev.evaluation_id,
            }"
            @click="handleSelect(ev)"
          >
            <div class="m-list-item__title">
              {{ ev.period || '—' }} · {{ ev.overall_score ?? '—' }}分
            </div>
            <div class="m-list-item__sub">{{ ev.status || '—' }}</div>
          </div>
          <div v-if="!evaluations.length" class="m-empty" style="padding: 16px">暂无已评估记录</div>
        </div>
      </div>
      ...
      <div class="m-field">
        ...
        <div class="m-seg">
          <button
            class="m-seg__item"
            :class="{ 'm-seg__item--active': form.type === 'feedback' }"
          ...
          <button
            class="m-seg__item"
            :class="{ 'm-seg__item--active': form.type === 'appeal' }"
            @click="form.type = 'appeal'"
          >
            申诉
          </button>
        </div>
    ...
    <div v-else class="m-list">
      ...
      >
        ...
        <div v-if="r.reply" class="m-list-item__sub" style="color: var(--el-color-primary)">
          回复：{{ r.reply }}
        </div>
      </div>
