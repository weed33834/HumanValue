      ...
      <div class="m-card">
        ...
        <label class="m-field">
          <span class="m-field__label"
            >日报内容 <i style="color: var(--el-color-danger)">*</i></span
          >
          <textarea
        ...
        <div class="m-field">
          ...
          <div v-if="attachments.length" style="margin-top: 8px">
            <div v-for="a in attachments" :key="a.uid" class="m-attach">
              ...
              <span
                class="m-tag"
                :class="{ 'm-tag--ok': a.status === 'success', 'm-tag--err': a.status === 'error' }"
              >
                {{
                  a.status === 'success'
                    ? '已上传'
              ...
              <button class="m-icon-btn" style="font-size: 16px" @click="removeAttachment(a.uid)">
                ✕
              </button>
            </div>
          </div>
