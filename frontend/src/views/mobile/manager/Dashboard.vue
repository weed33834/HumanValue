<template>
  <div class="m-page">
    ...
    <template v-else>
      <div class="m-stat-grid">
        ...
        <div class="m-stat">
          <div class="m-stat__value">
            {{ data.pending_approvals ?? data.pending_reviews ?? '—' }}
          </div>
          <div class="m-stat__label">待查看</div>
        </div>
      </div>
      ...
      <div class="m-list">
        <div v-for="m in modules" :key="m.path" class="m-list-item" @click="go(m.path)">
...
const modules = [
  { label: '团队分析', path: '/m/boss/team', icon: '◈' },
  { label: '智能助手', path: '/m/boss/assistant', icon: '💬' },
]
