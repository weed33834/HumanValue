<template>
  <div class="m-chat">
    <div ref="scrollEl" class="m-chat__msgs">
      <div v-if="chat.messages.length === 0 && !bootError" class="m-empty">开始与智能助手对话</div>
        ...
        <div class="m-msg__bubble">
          ...
          <template v-else>
            <div v-for="(part, i) in msg.parts" :key="i">
              ...
              <div v-else-if="part.type === 'image'" class="m-user-attach">
                🖼️ {{ part.metadata?.name || '图片' }}
              </div>
              <div v-else-if="part.type === 'file'" class="m-user-attach">
                📎 {{ part.metadata?.name || '文件' }}
              </div>
            </div>
          </template>
        </div>
...
watch(() => chat.messages.map((m) => m.streaming), scrollToBottom, { deep: true })
