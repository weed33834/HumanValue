...
describe('auth store', () => {
  ...
  it('loginDemo 设置 role/userId 并持久化到 localStorage', () => {
    ...
    expect(auth.useJwt).toBe(false)
    expect(localStorage.getItem('humanvalue_role')).toBe('manager')
    expect(localStorage.getItem('humanvalue_user_id')).toBe('M001')
    // demo 模式不应写 token
    expect(localStorage.getItem('humanvalue_token')).toBeNull()
  })
  ...
  it('loginWithToken 写入 token 并开启 useJwt', () => {
    ...
    expect(auth.isLoggedIn).toBe(true)
    expect(localStorage.getItem('humanvalue_token')).toBe(token)
  })
  ...
  it('logout 清空所有状态与 localStorage', async () => {
    ...
    expect(auth.useJwt).toBe(false)
    expect(auth.isLoggedIn).toBe(false)
    expect(localStorage.getItem('humanvalue_role')).toBeNull()
    expect(localStorage.getItem('humanvalue_token')).toBeNull()
  })
