    ...
    if (status === 401 && originalRequest && !originalRequest._retry && !isRefreshReq) {
      ...
      try {
        ...
        if (newToken) {
          ...
          localStorage.setItem('humanvalue_token', newToken)
        }
...
// 人才价值引擎 (老板/管理者视角: 九宫格/关键人/二八/人效/激励)
export const talentValueApi = {
  classification: () => api.get('/talent-value/classification'),
  criticalDependency: () => api.get('/talent-value/critical-dependency'),
  pareto: () => api.get('/talent-value/pareto'),
  efficiency: () => api.get('/talent-value/efficiency'),
  incentives: () => api.get('/talent-value/incentives'),
  marketCompetitiveness: () => api.get('/talent-value/market-competitiveness'),
  successionPipeline: () => api.get('/talent-value/succession-pipeline'),
  burnoutWarning: () => api.get('/talent-value/burnout-warning'),
  skillFit: () => api.get('/talent-value/skill-fit'),
  strategyReview: () => api.get('/talent-value/strategy-review'),
  systemTypes: () => api.get('/talent-value/system-types'),
  insights: () => api.get('/talent-value/insights'),
}
...
export const providerAdminApi = {
  ...
  getWorkspaceProvider: (provider) => api.get(`/admin/model-providers/providers/${provider}`),
  // 启用/禁用 Provider
...
export const insightsApi = {
  ...
  dashboard: (period, params) =>
    api.get(`/insights/dashboard/${encodeURIComponent(period)}`, { params }),
...
export const schedulerApi = {
  ...
  update: (taskId, payload) =>
    api.put(`/admin/scheduler/tasks/${encodeURIComponent(taskId)}`, payload),
  ...
  history: (taskId, params) =>
    api.get(`/admin/scheduler/tasks/${encodeURIComponent(taskId)}/history`, { params }),
}
...
export const ssoApi = {
  ...
  updateConfig: (configId, payload) =>
    api.put(`/admin/sso/configs/${encodeURIComponent(configId)}`, payload),
  ...
  callback: (configId, payload) =>
    api.post(`/admin/sso/configs/${encodeURIComponent(configId)}/callback`, payload),
  ldapLogin: (configId, payload) =>
    api.post(`/admin/sso/configs/${encodeURIComponent(configId)}/ldap-login`, payload),
}
...
export const sensitiveWordApi = {
  ...
  update: (wordId, payload) =>
    api.put(`/admin/sensitive-words/${encodeURIComponent(wordId)}`, payload),
  delete: (wordId) => api.delete(`/admin/sensitive-words/${encodeURIComponent(wordId)}`),
  review: (wordId, payload) =>
    api.post(`/admin/sensitive-words/${encodeURIComponent(wordId)}/review`, payload),
}
...
export const modelFallbackApi = {
  ...
  update: (chainId, payload) =>
    api.put(`/admin/model-fallback/chains/${encodeURIComponent(chainId)}`, payload),
...
export const modelLoadBalancerApi = {
  ...
  getInstance: (instanceId) =>
    api.get(`/admin/model-lb/instances/${encodeURIComponent(instanceId)}`),
  updateInstance: (instanceId, payload) =>
    api.put(`/admin/model-lb/instances/${encodeURIComponent(instanceId)}`, payload),
  deleteInstance: (instanceId) =>
    api.delete(`/admin/model-lb/instances/${encodeURIComponent(instanceId)}`),
  listConfigs: (params) => api.get('/admin/model-lb/configs', { params }),
  createConfig: (payload) => api.post('/admin/model-lb/configs', payload),
  updateConfig: (configId, payload) =>
    api.put(`/admin/model-lb/configs/${encodeURIComponent(configId)}`, payload),
  deleteConfig: (configId) => api.delete(`/admin/model-lb/configs/${encodeURIComponent(configId)}`),
  healthCheck: (instanceId) =>
    api.post(`/admin/model-lb/instances/${encodeURIComponent(instanceId)}/health-check`),
...
export const agentVersionApi = {
  ...
  // 版本操作
  listVersions: (agentId, params) =>
    api.get(`/admin/agents/${encodeURIComponent(agentId)}/versions`, { params }),
  createVersion: (agentId, payload) =>
    api.post(`/admin/agents/${encodeURIComponent(agentId)}/versions`, payload),
  getVersion: (agentId, versionId) =>
    api.get(
      `/admin/agents/${encodeURIComponent(agentId)}/versions/${encodeURIComponent(versionId)}`,
    ),
  publish: (agentId, versionId, payload) =>
    api.post(
      `/admin/agents/${encodeURIComponent(agentId)}/versions/${encodeURIComponent(versionId)}/publish`,
      payload,
    ),
  archive: (agentId, versionId) =>
    api.post(
      `/admin/agents/${encodeURIComponent(agentId)}/versions/${encodeURIComponent(versionId)}/archive`,
    ),
  rollback: (agentId, targetVersion) =>
    api.post(
      `/admin/agents/${encodeURIComponent(agentId)}/rollback/${encodeURIComponent(targetVersion)}`,
    ),
  compare: (agentId, v1Id, v2Id) =>
    api.get(
      `/admin/agents/${encodeURIComponent(agentId)}/versions/${encodeURIComponent(v1Id)}/compare/${encodeURIComponent(v2Id)}`,
    ),
}
...
export const publishApi = {
  ...
  update: (publishId, payload) =>
    api.put(`/admin/publish/${encodeURIComponent(publishId)}`, payload),
  // 渠道级操作
  status: (agentId) => api.get(`/admin/publish/${encodeURIComponent(agentId)}/status`),
  deploy: (agentId, channel, payload) =>
    api.post(
      `/admin/publish/${encodeURIComponent(agentId)}/${encodeURIComponent(channel)}`,
      payload,
    ),
  undeploy: (agentId, channel) =>
    api.delete(`/admin/publish/${encodeURIComponent(agentId)}/${encodeURIComponent(channel)}`),
}
...
export const grayReleaseApi = {
  ...
  update: (releaseId, payload) =>
    api.put(`/admin/gray-release/releases/${encodeURIComponent(releaseId)}`, payload),
  start: (releaseId) =>
    api.post(`/admin/gray-release/releases/${encodeURIComponent(releaseId)}/start`),
  pause: (releaseId) =>
    api.post(`/admin/gray-release/releases/${encodeURIComponent(releaseId)}/pause`),
  complete: (releaseId) =>
    api.post(`/admin/gray-release/releases/${encodeURIComponent(releaseId)}/complete`),
  rollback: (releaseId) =>
    api.post(`/admin/gray-release/releases/${encodeURIComponent(releaseId)}/rollback`),
  delete: (releaseId) =>
    api.delete(`/admin/gray-release/releases/${encodeURIComponent(releaseId)}`),
  stats: (releaseId) =>
    api.get(`/admin/gray-release/releases/${encodeURIComponent(releaseId)}/stats`),
  activeForAgent: (agentId) =>
    api.get(`/admin/gray-release/agents/${encodeURIComponent(agentId)}/active`),
}
...
export const environmentApi = {
  ...
  deploy: (envId, payload) =>
    api.post(`/admin/environments/${encodeURIComponent(envId)}/deploy`, payload),
...
export const datasetApi = {
  ...
  update: (datasetId, payload) =>
    api.put(`/admin/datasets/${encodeURIComponent(datasetId)}`, payload),
  delete: (datasetId) => api.delete(`/admin/datasets/${encodeURIComponent(datasetId)}`),
  listEntries: (datasetId, params) =>
    api.get(`/admin/datasets/${encodeURIComponent(datasetId)}/items`, { params }),
  createEntry: (datasetId, payload) =>
    api.post(`/admin/datasets/${encodeURIComponent(datasetId)}/items`, payload),
  updateEntry: (datasetId, itemId, payload) =>
    api.put(
      `/admin/datasets/${encodeURIComponent(datasetId)}/items/${encodeURIComponent(itemId)}`,
      payload,
    ),
  deleteEntry: (datasetId, itemId) =>
    api.delete(
      `/admin/datasets/${encodeURIComponent(datasetId)}/items/${encodeURIComponent(itemId)}`,
    ),
  importEntries: (datasetId, data) =>
    api.post(`/admin/datasets/${encodeURIComponent(datasetId)}/items/batch`, data),
  stats: (datasetId) => api.get(`/admin/datasets/${encodeURIComponent(datasetId)}/stats`),
  export: (datasetId, params) =>
    api.get(`/admin/datasets/${encodeURIComponent(datasetId)}/export`, {
      params,
      responseType: 'blob',
    }),
}
...
export const llmJudgeApi = {
  ...
  results: (taskId, params) =>
    api.get(`/admin/llm-judge/tasks/${encodeURIComponent(taskId)}/results`, { params }),
...
export const ragEvalApi = {
  ...
  results: (taskId, params) =>
    api.get(`/admin/rag-eval/tasks/${encodeURIComponent(taskId)}/results`, { params }),
...
export const annotationApi = {
  ...
  assign: (taskId, payload) =>
    api.post(`/admin/annotations/tasks/${encodeURIComponent(taskId)}/assign`, payload),
  annotate: (taskId, payload) =>
    api.post(`/admin/annotations/tasks/${encodeURIComponent(taskId)}/annotate`, payload),
  stats: (params) => api.get('/admin/annotations/stats', { params }),
  listAnnotations: (taskId, params) =>
    api.get(`/admin/annotations/tasks/${encodeURIComponent(taskId)}/annotations`, { params }),
  update: (taskId, payload) =>
    api.put(`/admin/annotations/tasks/${encodeURIComponent(taskId)}`, payload),
  batchFromEval: (payload) => api.post('/admin/annotations/tasks/batch-from-eval', payload),
  export: (taskId, params) =>
    api.get(`/admin/annotations/tasks/${encodeURIComponent(taskId)}/export`, {
      params,
      responseType: 'blob',
    }),
...
export const agentTemplateApi = {
  ...
  update: (templateId, payload) =>
    api.put(`/admin/agent-templates/${encodeURIComponent(templateId)}`, payload),
  ...
  instantiate: (templateId, payload) =>
    api.post(`/admin/agent-templates/${encodeURIComponent(templateId)}/install`, payload),
  market: (params) => api.get('/admin/agent-templates/market', { params }),
  reviews: (templateId, params) =>
    api.get(`/admin/agent-templates/${encodeURIComponent(templateId)}/reviews`, { params }),
  review: (templateId, payload) =>
    api.post(`/admin/agent-templates/${encodeURIComponent(templateId)}/review`, payload),
}
...
export const nl2sqlApi = {
  ...
  refreshSchema: (schemaId) =>
    api.post(`/admin/nl2sql/schemas/${encodeURIComponent(schemaId)}/refresh`),
  ...
  updateSchema: (schemaId, payload) =>
    api.put(`/admin/nl2sql/schemas/${encodeURIComponent(schemaId)}`, payload),
}
...
export const docParsingApi = {
  ...
  results: (taskId, params) =>
    api.get(`/admin/doc-parsing/tasks/${encodeURIComponent(taskId)}/results`, { params }),
...
export const graphRagApi = {
  ...
  entities: (taskId, params) =>
    api.get('/admin/graph-rag/entities', { params: { task_id: taskId, ...(params || {}) } }),
  relations: (taskId, params) =>
    api.get('/admin/graph-rag/relations', { params: { task_id: taskId, ...(params || {}) } }),
  getEntity: (entityId) => api.get(`/admin/graph-rag/entities/${encodeURIComponent(entityId)}`),
  entityRelations: (entityId, params) =>
    api.get(`/admin/graph-rag/entities/${encodeURIComponent(entityId)}/relations`, { params }),
  deleteEntity: (entityId) =>
    api.delete(`/admin/graph-rag/entities/${encodeURIComponent(entityId)}`),
  ...
  visualizeEntity: (entityId) =>
    api.get(`/admin/graph-rag/visualization/${encodeURIComponent(entityId)}`),
...
export const kbSyncApi = {
  ...
  updateSource: (sourceId, payload) =>
    api.put(`/admin/kb-sync/sources/${encodeURIComponent(sourceId)}`, payload),
  ...
  logs: (sourceId, params) =>
    api.get(`/admin/kb-sync/sources/${encodeURIComponent(sourceId)}/logs`, { params }),
}
...
export const promptOptimizationApi = {
  ...
  results: (taskId, params) =>
    api.get(`/admin/prompt-optimization/tasks/${encodeURIComponent(taskId)}/result`, { params }),
...
export const apiHealthApi = {
  ...
  updateSLO: (sloId, payload) =>
    api.put(`/admin/api-health/slo/${encodeURIComponent(sloId)}`, payload),
...
export const searchAdminApi = {
  ...
  incrementalUpdate: (documentId, payload) =>
    api.post(
      `/admin/search/documents/${encodeURIComponent(documentId)}/incremental-update`,
      payload,
    ),
}
...
export const toolConfigApi = {
  ...
  update: (toolName, payload) =>
    api.put(`/admin/tool-config/${encodeURIComponent(toolName)}/timeout`, payload),
...
export const talentApi = {
  // 目标管理 (OKR)
  createGoal: (payload) => api.post('/talent/goals', payload),
  listGoals: (params) => api.get('/talent/goals', { params }),
  getGoal: (goalId) => api.get(`/talent/goals/${goalId}`),
  updateGoal: (goalId, payload) => api.patch(`/talent/goals/${goalId}`, payload),
  deleteGoal: (goalId) => api.delete(`/talent/goals/${goalId}`),

  // 行动项追踪
  createActionItem: (payload) => api.post('/talent/action-items', payload),
  listActionItems: (params) => api.get('/talent/action-items', { params }),
  updateActionItem: (actionId, payload) => api.patch(`/talent/action-items/${actionId}`, payload),
  deleteActionItem: (actionId) => api.delete(`/talent/action-items/${actionId}`),
  actionItemsSummary: () => api.get('/talent/action-items/summary'),

  // 个人发展计划 (IDP)
  createIDP: (payload) => api.post('/talent/idps', payload),
  listIDPs: (params) => api.get('/talent/idps', { params }),
  getIDP: (planId) => api.get(`/talent/idps/${planId}`),
  updateIDP: (planId, payload) => api.patch(`/talent/idps/${planId}`, payload),

  // 1:1 会议
  createOneOnOne: (payload) => api.post('/talent/one-on-ones', payload),
  listOneOnOnes: (params) => api.get('/talent/one-on-ones', { params }),
  updateOneOnOne: (meetingId, payload) => api.patch(`/talent/one-on-ones/${meetingId}`, payload),

  // PIP 绩效改进
  createPIP: (payload) => api.post('/talent/pips', payload),
  listPIPs: (params) => api.get('/talent/pips', { params }),
  getPIP: (pipId) => api.get(`/talent/pips/${pipId}`),
  updatePIP: (pipId, payload) => api.patch(`/talent/pips/${pipId}`, payload),
}

// 员工敬业度 API
export const engagementApi = {
  // 脉搏调研
  createPulseSurvey: (payload) => api.post('/engagement/pulse/surveys', payload),
  listPulseSurveys: (params) => api.get('/engagement/pulse/surveys', { params }),
  submitPulseResponse: (payload) => api.post('/engagement/pulse/responses', payload),
  pulseAnalytics: (params) => api.get('/engagement/pulse/analytics', { params }),

  // 认可与奖励
  createRecognition: (payload) => api.post('/engagement/recognitions', payload),
  listRecognitions: (params) => api.get('/engagement/recognitions', { params }),
  recognitionLeaderboard: (params) => api.get('/engagement/recognitions/leaderboard', { params }),
}

// 继任规划 API
export const successionApi = {
  create: (payload) => api.post('/succession', payload),
  list: (params) => api.get('/succession', { params }),
  update: (planId, payload) => api.patch(`/succession/${planId}`, payload),
  summary: () => api.get('/succession/summary'),
}

// 人才成长 API
export const growthApi = {
  // 技能矩阵
  createSkill: (payload) => api.post('/growth/skills', payload),
  listSkills: (params) => api.get('/growth/skills', { params }),
  setEmployeeSkill: (payload) => api.post('/growth/employee-skills', payload),
  getEmployeeSkills: (employeeId) => api.get(`/growth/employee-skills/${employeeId}`),
  teamSkillMatrix: () => api.get('/growth/skill-matrix/team'),

  // 薪酬洞察
  createCompensation: (payload) => api.post('/growth/compensation', payload),
  listCompensation: (params) => api.get('/growth/compensation', { params }),
  compensationInsights: (params) => api.get('/growth/compensation/insights', { params }),

  // 内部人才流动
  createJobPosting: (payload) => api.post('/growth/job-postings', payload),
  listJobPostings: (params) => api.get('/growth/job-postings', { params }),
  applyInternal: (payload) => api.post('/growth/applications', payload),
  listApplications: (params) => api.get('/growth/applications', { params }),

  // 团队健康度
  teamHealth: () => api.get('/growth/team-health'),

  // 多期趋势
  employeeTrend: (employeeId, params) => api.get(`/growth/trends/${employeeId}`, { params }),
  teamTrendOverview: () => api.get('/growth/trends/team/overview'),
}
