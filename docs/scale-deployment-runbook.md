...
# 例: s3://humanvalue-attachments/ALI/eval-2026-07-001/screenshot_01.png
```
...
services:
  backend:
    build: ./backend
    environment:
      - DATABASE_URL=sqlite+aiosqlite:///./data/humanvalue.db
...
services:
  backend:
    build: ./backend
    environment:
      - DATABASE_URL=postgresql+asyncpg://humanvalue:***@postgres:5432/humanvalue
...
services:
  backend:
    ...
    deploy:
      replicas: 2
    environment:
      - DATABASE_URL=postgresql+asyncpg://humanvalue:***@pg-primary:5432/humanvalue
      - DATABASE_REPLICA_URL=postgresql+asyncpg://humanvalue:***@pg-replica:5432/humanvalue
...
spec:
  ...
  template:
    spec:
      containers:
        - name: backend
          image: humanvalue-backend:v1.0.0
          resources:
