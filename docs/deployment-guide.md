...
> **目标：** 指导企业完成 HumanValue 的内网/云端/混合部署
...
| 服务 | 容器名 | 端口 | 说明 |
|---|---|---|---|
| backend | humanvalue-backend | 8000 | FastAPI 后端 |
| frontend | humanvalue-frontend | 80 | Vue3 前端（Nginx） |
| redis | humanvalue-redis | 6379 | 缓存 |

### 3.3 持久化卷

| 卷名 | 用途 |
|---|---|
| humanvalue_db_data | SQLite 数据库 |
| humanvalue_chroma_data | 向量库 |
| humanvalue_redis_data | Redis 数据 |
| humanvalue_attachments | 本地附件（如上传的截图、语音、PDF） |
...
S3_BUCKET=humanvalue-attachments
S3_SECURE=false
```
...
| `S3_BUCKET` | 默认 `humanvalue-attachments`；不存在时后端首次连接自动创建 |
...
```env
DATABASE_URL=postgresql+asyncpg://user:password@postgres:5432/humanvalue
```
...
```yaml
  postgres:
    ...
    container_name: humanvalue-postgres
    environment:
      POSTGRES_USER: user
      POSTGRES_PASSWORD: password
      POSTGRES_DB: humanvalue
    volumes:
      - humanvalue_pg_data:/var/lib/postgresql/data
...
```yaml
  minio:
    ...
    container_name: humanvalue-minio
    command: server /data --console-address ":9001"
    environment:
      MINIO_ROOT_USER: minioadmin
      MINIO_ROOT_PASSWORD: minioadmin
    volumes:
      - humanvalue_minio_data:/data
    ports:
      - "9000:9000"
      - "9001:9001"
    restart: unless-stopped
...
server {
    ...
    server_name humanvalue.example.com;
    ...
    location / {
        proxy_pass http://humanvalue-frontend;
    }

    location /api/ {
        proxy_pass http://humanvalue-backend:8000;
...
- **Grafana** `http://localhost:3000`：启动自动加载 Prometheus 数据源 + HumanValue 看板（provisioning 免手动 Import）；prod compose 强制必填 `GRAFANA_ADMIN_USER` / `GRAFANA_ADMIN_PASSWORD`（无默认值，未设置则容器启动失败）
...
- 项目仓库：https://gitcode.com/badhope/HumanValue
- 问题反馈：通过仓库 Issue 提交
